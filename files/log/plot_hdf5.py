...
def lag_finder(y1, y2, sr):
    ...
    corr = signal.correlate(y2.flatten(), y1.flatten())
    lags = signal.correlation_lags(len(y2.flatten()), len(y1.flatten())).flatten()
    corr /= np.max(corr).flatten()

    print(corr)

    fig, (ax_orig, ax_noise, ax_corr) = plt.subplots(3, 1, figsize=(4.8, 4.8))
    ax_orig.plot(y1.flatten())
    ax_orig.set_title('y1 signal')
    ax_orig.set_xlabel('Sample Number')
    ax_noise.plot(y2.flatten())
    ax_noise.set_title('y2 Signal')
...
def verify_hdf5(hdf5, frame_i=100, cell_i=0, ofdm_sym_i=0, ant_i =0,
    ...
    if pilot_data_avail:
        ...
        for i in insp_ants:
            ...
            pilot_frames = [i for i in range(len(amps)) if amps[i] > 0.00]
    ...
    if ul_data_avail > 0:
        ...
        print("finished ul plotting!!!\n")

        certain_frame_iq = ul_samps[ref_frame, ul_slot_i, ant_i, :]
        certain_frame_iq = certain_frame_iq.reshape(1376,1)
        check_aa = certain_frame_iq.reshape(1, 1376)
        check_aa = np.asarray(check_aa)
        # print(certain_frame_iq)
        # print(certain_frame_iq.shape)
        noise_part_1 = np.asarray(certain_frame_iq[0:140])
        noise_part_1 = noise_part_1.reshape(1,140)
        # print(noise_part_1.shape)
        noise_part_2 = np.asarray(certain_frame_iq[1300:1376])
        noise_part_2 = noise_part_2.reshape(1, 76)
        # print(noise_part_2.shape)
        signal_part = np.asarray(certain_frame_iq[200:700])
        signal_part = signal_part.reshape(1, 500)
        # print(signal_part.shape)

        aaa = np.mean(np.power(np.real(signal_part),2))
        bbb= np.mean(np.power(np.imag(signal_part),2))
        ccc = aaa + bbb;
        signal_p_db = 10 * np.log10(ccc)
        # print(ccc)

        n_aaa = np.power(np.real(noise_part_1), 2)
        n_bbb = np.power(np.imag(noise_part_1), 2)
        n_ccc = n_aaa + n_bbb;

        n_aaa_2 = np.power(np.real(noise_part_2), 2)
        n_bbb_2 = np.power(np.imag(noise_part_2), 2)
        n_ccc_2 = n_aaa_2 + n_bbb_2;

        noise_p = (np.mean(n_ccc) + np.mean(n_ccc_2 )) / 2 
        noise_p_db = 10 * np.log10(noise_p)

        print("new SNR is:")
        print(signal_p_db - noise_p_db)



        # certain_frame_iq_bb = ul_samps[ref_frame + 0, 0, ant_i, :]
        # certain_frame_iq_bb = certain_frame_iq_bb.reshape(896,1)
        # check_bb = certain_frame_iq_bb.reshape(1, 896)
        # check_bb = np.asarray(check_bb)



        # lag_finder(np.real(check_aa), np.real(check_bb), 200000000)

        # SNR = signal_power / noise_power
        # print("SNR is: ")
        # print(10 * np.log10(SNR))

        # print("SNR is: %f", snr_ul)
