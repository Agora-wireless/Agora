...
std::vector<std::complex<float>> RadioConfigNoRxStream::SnoopSamples(
...
void RadioConfigNoRxStream::AdjustCalibrationGains(
    std::vector<SoapySDR::Device*> rxDevs, SoapySDR::Device* txDev,
    size_t channel, double fftBin, bool plot) {
  using std::cout;
  using std::endl;
  ...
  for (size_t r = 0; r < rx_devs_size; r++) {
    const auto samps =
        RadioConfigNoRxStream::SnoopSamples(rxDevs[r], channel, n);
  ...
  for (size_t r = 0; r < rx_devs_size; r++) {
    ...
    auto samps = RadioConfigNoRxStream::SnoopSamples(rxDevs[r], channel, n);
    float tone_level =
        CommsLib::MeasureTone(samps, win, window_gain, fftBin, n);
    if (tone_level > target_level) {
      adjusted_radios[r] = true;
...
void RadioConfigNoRxStream::SetIqBalance(SoapySDR::Device* dev, int direction,
                                         size_t channel, int gcorr,
                                         int iqcorr) {
...
void RadioConfigNoRxStream::DciqMinimize(SoapySDR::Device* targetDev,
  ...
  for (size_t iter = 0; iter < 4; iter++) {
    ...
    for (int i = start; i < stop; i += step) {
      ...
      RadioConfigNoRxStream::SetIqBalance(targetDev, direction, channel, gcorr,
                                          iqcorr);
  ...
  RadioConfigNoRxStream::SetIqBalance(targetDev, direction, channel, bestgcorr,
                                      bestiqcorr);
...
void RadioConfigNoRxStream::DciqCalibrationProc(size_t channel) {
  std::cout << "****************************************************\n";
  ...
  RadioConfigNoRxStream::AdjustCalibrationGains(
  ...
  for (size_t r = 0; r < radio_size - 1; r++) {
    RadioConfigNoRxStream::DciqMinimize(
        all_but_ref_devs[r], all_but_ref_devs[r], SOAPY_SDR_RX, channel, 0.0,
  ...
  RadioConfigNoRxStream::DciqMinimize(ref_dev, ref_dev, SOAPY_SDR_RX, channel,
  ...
  RadioConfigNoRxStream::DciqMinimize(ref_dev, ref_ref_dev, SOAPY_SDR_TX,
  ...
  for (size_t r = 0; r < radio_size - 1; r++) {
    ...
    RadioConfigNoRxStream::AdjustCalibrationGains(
        ...
        (tone_bb_freq + tx_tone_bb_freq) / sample_rate);
    RadioConfigNoRxStream::DciqMinimize(
        all_but_ref_devs[r], ref_dev, SOAPY_SDR_TX, channel,
...
void RadioConfigNoRxStream::AdjustDelays(std::vector<int> offset) {
...
bool RadioConfigNoRxStream::InitialCalib(bool sample_adjust) {
