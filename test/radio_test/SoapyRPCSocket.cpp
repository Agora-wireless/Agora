...
void sklk_SoapyRPCSocket::setDefaultTcpSockOpts(void) {
  ...
  int one = 1;
  int ret = ::setsockopt(_sock, IPPROTO_TCP, TCP_NODELAY, (const char *)&one,
                         sizeof(one));
  if (ret != 0) {
...
#ifdef TCP_QUICKACK
  ret = ::setsockopt(_sock, IPPROTO_TCP, TCP_QUICKACK, (const char *)&one,
                     sizeof(one));
  if (ret != 0) {
...
sklk_SoapyRPCSocket::sklk_SoapyRPCSocket(void) : _sock(INVALID_SOCKET) {
  return;
}
...
sklk_SoapyRPCSocket::~sklk_SoapyRPCSocket(void) {
  if (this->close() != 0) {
    SoapySDR::logf(SOAPY_SDR_ERROR,
                   "sklk_SoapyRPCSocket::~sklk_SoapyRPCSocket: %s",
                   this->lastErrorMsg());
  }
}

bool sklk_SoapyRPCSocket::null(void) { return _sock == INVALID_SOCKET; }
...
int sklk_SoapyRPCSocket::bind(const std::string &url) {
  ...
  //setup reuse address
  int one = 1;
  int ret = ::setsockopt(_sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&one,
                         sizeof(one));
  if (ret != 0) {
...
#endif
  if (ret != 0)
    this->reportError("setNonBlocking(" +
                      std::string(nonblock ? "true" : "false") + ")");
  return ret;
}
...
static int getDefaultIfaceIndex(void) {
  ...
  while (ifa != nullptr) {
    ...
    SoapySDR::logf(
        SOAPY_SDR_DEBUG, "Interface: #%d(%s) ipv6=%d, up=%d, lb=%d, mcast=%d",
        ifaceIndex, ifa->ifa_name, isIPv6, isUp, isLoopback, isMulticast);
    if (isIPv6 and isUp and isLoopback and isMulticast and loIface == 0)
      loIface = ifaceIndex;
    if (isIPv6 and isUp and not isLoopback and isMulticast and enIface == 0)
      enIface = ifaceIndex;
    ifa = ifa->ifa_next;
  }
  freeifaddrs(ifa);
  SoapySDR::logf(SOAPY_SDR_DEBUG, "Default loopback: #%d, default ethernet #%d",
                 loIface, enIface);
...
int sklk_SoapyRPCSocket::multicastJoin(const std::string &group,
                                       const bool loop, const int ttl,
                                       int iface) {
  /*
     * Multicast join docs:
  ...
  switch (addr.addr()->sa_family) {
    case AF_INET: {
      ...
      ret = ::setsockopt(_sock, IPPROTO_IP, IP_MULTICAST_LOOP,
                         (const char *)&loopInt, sizeof(loopInt));
      if (ret != 0) {
      ...
      ret = ::setsockopt(_sock, IPPROTO_IP, IP_MULTICAST_TTL,
      ...
      ret = ::setsockopt(_sock, IPPROTO_IP, IP_ADD_MEMBERSHIP,
    ...
    case AF_INET6: {
      ...
      ret = ::setsockopt(_sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP,
                         (const char *)&loopInt, sizeof(loopInt));
      if (ret != 0) {
      ...
      ret = ::setsockopt(_sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS,
                         (const char *)&ttl, sizeof(ttl));
      if (ret != 0) {
      ...
      if (iface != 0) {
        ret = ::setsockopt(_sock, IPPROTO_IPV6, IPV6_MULTICAST_IF,
      ...
      ret = ::setsockopt(_sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
...
int sklk_SoapyRPCSocket::sendto(const void *buf, size_t len,
                                const std::string &url, int flags) {
  SockAddrData addr;
  SoapyURL(url).toSockAddr(addr);
  int ret = ::sendto(_sock, (char *)buf, int(len), flags, addr.addr(),
                     addr.addrlen());
  if (ret == -1) this->reportError("sendto(" + url + ")");
  return ret;
}

int sklk_SoapyRPCSocket::recvfrom(void *buf, size_t len, std::string &url,
                                  int flags) {
  struct sockaddr_storage addr;
  socklen_t addrlen = sizeof(addr);
  int ret = ::recvfrom(_sock, (char *)buf, int(len), flags,
                       (struct sockaddr *)&addr, &addrlen);
  if (ret == -1)
    this->reportError("recvfrom()");
  else
...
void sklk_SoapyRPCSocket::reportError(const std::string &what, const int err) {
  if (err == 0)
    ...
    this->reportError(what, std::to_string(err) + ": " + errToString(err));
}
...
int sklk_SoapyRPCSocket::setBuffSize(const bool isRecv, const size_t numBytes) {
  ...
  int ret = ::setsockopt(_sock, SOL_SOCKET, isRecv ? SO_RCVBUF : SO_SNDBUF,
                         (const char *)&opt, sizeof(opt));
  if (ret == -1)
    this->reportError("setsockopt(" +
                      std::string(isRecv ? "SO_RCVBUF" : "SO_SNDBUF") + ")");
  return ret;
}
...
int sklk_SoapyRPCSocket::getBuffSize(const bool isRecv) {
  int opt = 0;
  socklen_t optlen = sizeof(opt);
  int ret = ::getsockopt(_sock, SOL_SOCKET, isRecv ? SO_RCVBUF : SO_SNDBUF,
                         (char *)&opt, &optlen);
  if (ret == -1)
    this->reportError("getsockopt(" +
                      std::string(isRecv ? "SO_RCVBUF" : "SO_SNDBUF") + ")");
  if (ret != 0) return ret;
...
#ifdef SO_PRIORITY
  int opt = priority;
  int ret = ::setsockopt(_sock, SOL_SOCKET, SO_PRIORITY, (const char *)&opt,
                         sizeof(opt));
