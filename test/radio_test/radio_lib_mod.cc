...
void RadioConfigNoRxStream::DrainBuffers() {
  ...
  for (size_t i = 0; i < cfg_->NumRadios(); i++) {
    RadioConfigNoRxStream::DrainRxBuffer(ba_stn_.at(i), rx_streams_.at(i),
                                         rx_buffs, cfg_->SampsPerSymbol());
  }
}

void RadioConfigNoRxStream::DrainRxBuffer(SoapySDR::Device* ibsSdrs,
