...
static double bench_mod_16qam(unsigned iterations, unsigned mode)
{
    ...
    for (unsigned i = 0; i < iterations; i++) {
        ...
        if (mode == 0) {
            ...
            demod_16qam_soft_avx2((float *)output_mod, (int8_t *)output_demod_avx2, num);
        }
    }
    ...
    if (mode == 1) {
        ...
        for (unsigned i = 0; i < num * 4; i++) {
            if (output_demod_sse[i] != output_demod_avx2[i]) {
                num_error++;
                // printf("error at %d: %i, %i\n", i, (int8_t)output_demod_sse[i], (int8_t)output_demod_avx2[i]);
            }
        }
...
static double bench_mod_64qam(unsigned iterations, unsigned mode)
{
    ...
    for (unsigned i = 0; i < iterations; i++) {
        ...
        if (mode == 0) {
            ...
            demod_64qam_soft_avx2((float *)output_mod, (int8_t *)output_demod_avx2, num);
        }
    }
    double end_time = get_time();
...
{
    if (argc != 4) {
        fprintf(stderr, "Usage: %s [modulation order 4/16/64] [mode hard(0)/soft(1)] [iterations]\n",
                argv[0]);
        return 1;
    }
