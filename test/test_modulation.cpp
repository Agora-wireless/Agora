  ...
  for (unsigned i = 0; i < iterations; i++) {
    ...
    if (mode == 0) {
      ...
      demod_16qam_soft_loop((float *)output_mod, (int8_t *)output_demod_loop,
                            num);
      demod_16qam_soft_sse((float *)output_mod, (int8_t *)output_demod_sse,
                           num);
  ...
  if (mode == 1) {
    ...
    for (unsigned i = 0; i < num * 4; i++) {
      if (output_demod_sse[i] != output_demod_avx2[i]) {
        num_error++;
        // printf("error at %d: %i, %i\n", i, (int8_t)output_demod_sse[i],
        // (int8_t)output_demod_avx2[i]);
      }
    }
  ...
  for (unsigned i = 0; i < iterations; i++) {
    ...
    if (mode == 0) {
      ...
      demod_64qam_soft_loop((float *)output_mod, (int8_t *)output_demod_loop,
                            num);
      demod_64qam_soft_sse((float *)output_mod, (int8_t *)output_demod_sse,
                           num);
