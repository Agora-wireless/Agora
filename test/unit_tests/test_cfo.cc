...
TEST(TestCFO, EstCorr)
{
    ...
    double ms0 = GetTime::CyclesToMs(GetTime::Rdtsc() - start_tsc, cfg->FreqGhz());

    double cfo_before_corr = ru_->GetCFO();
    ...
    double ms1 = GetTime::CyclesToMs(GetTime::Rdtsc() - start_tsc, cfg->FreqGhz());
