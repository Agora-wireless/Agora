...
TEST(TestRecip, Correctness)
{
    ...
    for (size_t i = 0; i < kMaxFrameNum; i++) {
        ...
        arma::cx_fmat mat_input(
            ptr_in, cfg->ofdm_data_num_, cfg->bs_ant_num_, false);
        ...
        arma::cx_fmat recip_mat(
            recip_buff, cfg->bs_ant_num_, cfg->ofdm_data_num_, false);
        ...
        for (size_t sc_id = 0; sc_id < cfg->ofdm_data_num_;
            ...
            recip_mat
                .cols(sc_id,
                    std::min(
                        sc_id + cfg->bs_ant_num_ - 1, cfg->ofdm_data_num_ - 1))
                .each_col()
                = recip_mat.col(sc_id);
        }
    }
    ...
    for (size_t i = 0; i < kMaxFrameNum; i++) {
        ...
        for (size_t ant_id = 0; ant_id < cfg->bs_ant_num_; ant_id++) {
            auto* ptr_in
                = calib_buffer[i % kFrameWnd] + ant_id * cfg->ofdm_data_num_;
            for (size_t sc_id = 0; sc_id < cfg->ofdm_data_num_;
                 sc_id += cfg->bs_ant_num_) {
        ...
        arma::cx_fmat calib_mat(
            ptr_calib, cfg->ofdm_data_num_, cfg->bs_ant_num_, false);
        arma::cx_float* recip_buff
