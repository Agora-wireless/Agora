...
void equal_org(Config* cfg_,
  ...
  mkl_jit_status_t status =
      mkl_jit_create_cgemm(&jitter_, MKL_COL_MAJOR, MKL_NOTRANS, MKL_NOTRANS,
                           cfg_->UeAntNum(), 1, cfg_->BsAntNum(),
                           &alpha, cfg_->UeAntNum(), cfg_->BsAntNum(),
                           &beta, cfg_->UeAntNum());
...
TEST(TestEqual, OrgSingle) {
  ...
  equal_buffer_.RandAllocCxFloat(task_buffer_symbol_num_ul,
                       cfg_->OfdmDataNum() * cfg_->UeAntNum(),
                       Agora_memory::Alignment_t::kAlign64);
  ue_spec_pilot_buffer_.RandAllocCxFloat(
      kFrameWnd,
      cfg_->Frame().ClientUlPilotSymbols() * cfg_->UeAntNum(),
      Agora_memory::Alignment_t::kAlign64);

  ul_beam_matrices_.RandAllocCxFloat(cfg_->BsAntNum() * cfg_->UeAntNum());
...
TEST(TestEqual, CorrectnessSingle) {
  ...
  // PtrGrid<kFrameWnd, kMaxDataSCs, complex_float> ul_beam_matrices_(
  //   kFrameWnd, cfg_->OfdmDataNum(), cfg_->BsAntNum() * cfg_->UeAntNum());
  PtrGrid<kFrameWnd, kMaxDataSCs, complex_float> ul_beam_matrices_;
