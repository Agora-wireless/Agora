...
static double bench_mod_16qam(unsigned iterations, unsigned mode)
{
    ...
    for (unsigned i = 0; i < iterations; i++) {
        ...
        if (mode == 0) {
            ...
            demod_16qam_soft_loop(
                (float*)output_mod, (int8_t*)output_demod_loop, num);
            demod_16qam_soft_sse(
                (float*)output_mod, (int8_t*)output_demod_sse, num);
            demod_16qam_soft_avx2(
                (float*)output_mod, (int8_t*)output_demod_avx2, num);
        }
    }
