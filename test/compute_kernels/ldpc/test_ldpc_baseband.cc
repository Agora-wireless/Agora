...
int main(int argc, char* argv[])
{
    ...
    for (size_t noise_id = 0; noise_id < 15; noise_id++) {
        ...
        size_t num_unused_symbol
            = cfg->data_symbol_num_perframe_ - num_used_symbol;
        ...
        demod_data_all_symbols.Calloc(cfg->ue_ant_num_,
        ...
        for (size_t i = data_sym_start; i < cfg->symbol_num_perframe_; i++) {
            ...
            for (size_t j = 0; j < cfg->ofdm_data_num_; j++) {
                arma::cx_fmat mat_precoder(
                    reinterpret_cast<arma::cx_float*>(
                        precoder[cfg->freq_orthogonal_pilot_
                                ? (j % cfg->ue_ant_num_)
                                : j]),
                    cfg->ue_ant_num_, cfg->bs_ant_num_, false);
                mat_equalized_data.row(j) = (mat_precoder
                    * mat_rx_data.row(j + cfg->ofdm_data_start_).st())
                                                .st();
            }
            ...
            for (size_t j = 0; j < cfg->ue_ant_num_; j++) {
                ...
                case (4):
                    Demod16qamSoftAvx2(
                        equal_t_ptr, demod_ptr, cfg->ofdm_data_num_);
                    break;
                case (6):
                    Demod64qamSoftAvx2(
                        equal_t_ptr, demod_ptr, cfg->ofdm_data_num_);
