    ...
    for (int i = 0; i < LOOP_NUM; ++i) {
        ...
        for (int c1 = 0; c1 < OFDM; c1++) {
            for (int c2 = 0; c2 < BS_ANT / 4; c2++) {
                ...
                __m256d t_data
                    = _mm256_i64gather_pd((double*)base_ptr, index, 8);
        ...
        for (int c1 = 0; c1 < BS_ANT; c1++) {
            for (int c2 = 0; c2 < OFDM / BLOCK * 2; c2++) {
                for (int c3 = 0; c3 < BLOCK / 8; c3++) {
                    __m256 data = _mm256_load_ps(
                        src_ptr + c1 * OFDM * 2 + c2 * BLOCK + c3 * 8);
                    _mm256_store_ps(
                        tar_ptr + c2 * BS_ANT * BLOCK + BLOCK * c1 + c3 * 8,
                        data);
                }
            }
        }
    ...
    for (int i = 0; i < LOOP_NUM; ++i) {
        ...
        for (int c1 = 0; c1 < OFDM; c1++) {
            for (int c2 = 0; c2 < BS_ANT / 4; c2++) {
                ...
                float* base_ptr = tar_ptr + c1_base * BLOCK * BS_ANT + c1_offset
                    + c2 * BLOCK * 4;
                __m256d t_data
                    = _mm256_i64gather_pd((double*)base_ptr, index, 8);
                _mm256_store_pd((double*)(temp_buffer_ptr + c2 * 8), t_data);
