...
if [ "${bigstation_mode}" == "true" ]; then
  ...
  if [ ${frame_str} == '"PDDDDDDDDDDDDD"' ]; then
    num_ifft_workers=$(cat ${HYDRA_SERVER_DEPLOY_JSON} | jq '.num_ifft_workers')
    num_zf_workers=$(cat ${HYDRA_SERVER_DEPLOY_JSON} | jq '.num_zf_workers')
    num_precode_workers=$(cat ${HYDRA_SERVER_DEPLOY_JSON} | jq '.num_precode_workers')
    num_encode_workers=$(cat ${HYDRA_SERVER_DEPLOY_JSON} | jq '.num_encode_workers')
    for (( i=0; i<${hydra_rru_num}; i++ )) do
      cat ${HYDRA_SYSTEM_CONFIG_JSON} | jq --argjson list "${num_ifft_workers}" '.num_ifft_workers=$list' | \
        jq --argjson list "${num_zf_workers}" '.num_zf_workers=$list' | \
        jq --argjson list "${num_precode_workers}" '.num_precode_workers=$list' | \
        jq --argjson list "${num_encode_workers}" '.num_encode_workers=$list' > tmp_0.json
  ...
  fft_thread_list=$(cat ${HYDRA_SERVER_DEPLOY_JSON} | jq '.fft_thread_num')
  for (( i=0; i<${hydra_rru_num}; i++ )) do
    cat ${HYDRA_SYSTEM_CONFIG_JSON} | jq --argjson list "${sc_num_list}" '.subcarrier_num_list=$list' | \
      ...
      jq --argjson list "${cd_thread_list}" '.coding_thread_num=$list' | \
      jq --argjson list "${fft_thread_list}" '.fft_thread_num=$list' > tmp_0.json
    for (( j=0; j<${hydra_rru_num}; j++ )) do
      server_name=$(cat ${HYDRA_SERVER_DEPLOY_JSON} | jq --argjson j $j '.rru_servers[$j]' | tr -d '"')
      unset list_idx
      for (( k=0; k<${hydra_server_num}; k++ )) do
