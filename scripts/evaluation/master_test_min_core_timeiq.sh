...
for i in ${!ant_list[@]}; do
    ...
    HYDRA_SYSTEM_CONFIG_JSON=config/template_msr/template_msr_${ant_num}_${ue_num}_timeiq.json
    HYDRA_SERVER_DEPLOY_JSON=config/deploy_msr/deploy_msr_${ant_num}_${ue_num}_timeiq.json
    ...
    ${hydra_root_dir}/scripts/evaluation/test_min_core_timeiq.sh
done