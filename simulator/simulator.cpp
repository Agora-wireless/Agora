...
Simulator::Simulator(Config *cfg, int in_task_thread_num, int in_socket_tx_num,
    ...
    : TASK_THREAD_NUM(in_task_thread_num),
      ...
      SOCKET_TX_THREAD_NUM(in_socket_tx_num), CORE_OFFSET(in_core_offset) {
  ...
  printf("new Sender\n");
  sender_.reset(
      new Sender(config_, SOCKET_TX_THREAD_NUM, CORE_OFFSET + 1, sender_delay));

  printf("new Receiver\n");
  receiver_.reset(new Receiver(
      config_, SOCKET_RX_THREAD_NUM, SOCKET_TX_THREAD_NUM, CORE_OFFSET,
...
void Simulator::start() {
  ...
  std::vector<pthread_t> rx_threads = receiver_->startRecv(
      ...
      socket_buffer_size_, frame_start);
  ...
  while (config_->running && !SignalHandler::gotExitSignal()) {
    ...
    for (int bulk_count = 0; bulk_count < ret; bulk_count++) {
      ...
      case EVENT_PACKET_RECEIVED: {
        ...
        interpreteOffset2d_setbits(offset, &socket_thread_id,
                                   &offset_in_current_buffer, 28);
  ...
  for (int ii = 0; ii < frame_count_rx; ii++) {
    fprintf(fp, "%.3f %.3f %.3f %.3f\n", frame_start[0][ii], frame_start[1][ii],
            frame_start_receive[ii], frame_end_receive[ii]);
  }
  exit(0);
}
...
void Simulator::update_rx_counters(int frame_id, int frame_id_in_buffer,
                                   int subframe_id, int ant_id) {
  ...
  if (rx_counter_packets_[frame_id_in_buffer] == 1) {
    ...
    printf("Main thread: data received from frame %d, subframe %d, ant %d, in "
...
void Simulator::print_per_frame_done(int task_type, int frame_id,
                                     int frame_id_in_buffer) {
#if DEBUG_PRINT_PER_FRAME_DONE
  switch (task_type) {
  case (PRINT_RX): {
    printf("Main thread: received all packets in frame: %d, in %.2f us since "
...
void Simulator::initialize_queues() {
  message_queue_ =
      moodycamel::ConcurrentQueue<Event_data>(512 * data_subframe_num_perframe);
...
void Simulator::initialize_uplink_buffers() {
  ...
  socket_buffer_status_.calloc(SOCKET_RX_THREAD_NUM, socket_buffer_status_size_,
                               64);
