...
void Simulator::start()
{
    ...
    for(int ii = 0; ii < frame_count_rx; ii++) {    
        fprintf(fp, "%.3f %.3f %.3f %.3f\n", frame_start[0][ii], frame_start[1][ii], frame_start_receive[ii], frame_end_receive[ii]);
    }
    exit(0);
}
...
void Simulator::update_rx_counters(int frame_id, int frame_id_in_buffer, int subframe_id, int ant_id)
{
    ...
    if (rx_counter_packets_[frame_id_in_buffer] == 1) {   
        ...
        printf("Main thread: data received from frame %d, subframe %d, ant %d, in %.2f since tx, in %.2f us since last frame\n", \
                frame_id, subframe_id, ant_id, frame_start_receive[frame_id] - frame_start_tx[frame_id],
...
void Simulator::print_per_frame_done(int task_type, int frame_id, int frame_id_in_buffer)
{
