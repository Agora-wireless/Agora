    ...
    if (num_tags > 0) {
      for (size_t tag_id = 0; (tag_id < num_tags); tag_id++) {
        ...
        if (kDebugSenderReceiver) {
          AGORA_LOG_INFO(
              "Thread %d (tag = %s) transmit frame %d, symbol %d, bs ant id %d, "
              "adapt ue(s) %zu, pkt size %zu, dest port %zu, TX time: %.3f us\n",
              tid, gen_tag_t(tag).ToString().c_str(), pkt->frame_id_,
              pkt->symbol_id_, pkt->ant_id_, adapt_ues_array_.at(pkt->frame_id_),
...
void Sender::InitUesFromFile(const std::string& filepath) {
  ...
  const std::string filename = filepath +
    "/files/experiment/adapt" +
    "_ueant" + std::to_string(cfg_->UeAntNum()) + ".bin";

  AGORA_LOG_INFO("Sender: Reading adaptable number of UEs across frames from %s\n",
                  filename.c_str());
...
void Sender::InitIqFromFilePath(const std::string& filepath) {
  ...
  iq_data_float.Calloc(packets_per_frame * cfg_->UeAntNum(),
                       cfg_->SampsPerSymbol() * 2,
                       Agora_memory::Alignment_t::kAlign64);
  ...
  for (size_t ue_ant_id = 1; ue_ant_id <= cfg_->UeAntNum(); ue_ant_id++) {
    ...
    for (size_t i = 0; i < packets_per_frame; i++) {
      ...
      if (kUse12BitIQ) {
        ...
        ConvertFloatTo12bitIq(iq_data_float[packets_per_frame * (ue_ant_id - 1) + i],
                              reinterpret_cast<uint8_t*>(iq_data_short_[packets_per_frame * (ue_ant_id - 1) + i]),
                              expected_count);
      } else {
        SimdConvertFloatToShort(iq_data_float[packets_per_frame * (ue_ant_id - 1) + i],
