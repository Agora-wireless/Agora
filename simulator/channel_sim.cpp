...
ChannelSim::ChannelSim(Config* config_bs, Config* config_ue,
  ...
  srand(time(NULL));
  dl_data_plus_beacon_symbols = bscfg->frame().NumDLSyms() + 1;  // plus beacon
  ...
  message_queue_ = moodycamel::ConcurrentQueue<Event_data>(
      kFrameWnd * bscfg->frame().NumTotalSyms() *
...
void ChannelSim::schedule_task(
    Event_data do_task, moodycamel::ConcurrentQueue<Event_data>* in_queue,
    ...
    for (int bulk_count = 0; bulk_count < ret; bulk_count++) {
      ...
      switch (event.event_type) {
        case EventType::kPacketRX: {
          ...
          if (gen_tag_t(event.tags[0]).tag_type == gen_tag_t::TagType::kUsers) {
            ...
            if (user_rx_counter_[frame_offset] == uecfg->ue_ant_num()) {
              ...
              if (kDebugPrintPerSymbolDone)
                std::printf(
                    ...
                    bscfg->bs_ant_num());
              schedule_task(Event_data(EventType::kPacketTX, event.tags[0]),
        ...
        case EventType::kPacketTX: {
          ...
          if (gen_tag_t(event.tags[0]).tag_type == gen_tag_t::TagType::kUsers) {
            ...
            if (user_tx_counter_[offset] == dl_data_plus_beacon_symbols) {
...
void* ChannelSim::taskThread(int tid) {
  pin_to_core_with_offset(ThreadType::kWorker,
                          core_offset + bs_thread_num + 1 + user_thread_num,
                          tid);
...
void* ChannelSim::bs_rx_loop(int tid) {
  ...
  while (running) {
    ...
    std::memcpy(&rx_buffer_bs[offset * payload_length], pkt->data,
                payload_length);
...
void* ChannelSim::ue_rx_loop(int tid) {
  ...
  pin_to_core_with_offset(ThreadType::kWorkerTXRX,
                          core_offset + 1 + bs_thread_num, tid);
  ...
  while (running) {
    ...
    size_t pilot_symbol_id = uecfg->GetPilotSymbolIdx(frame_id, symbol_id);
    size_t ul_symbol_id = uecfg->GetULSymbolIdx(frame_id, symbol_id);
    size_t total_symbol_id = pilot_symbol_id;
    if (pilot_symbol_id == SIZE_MAX)
      total_symbol_id = ul_symbol_id + bscfg->frame().NumPilotSyms();
    ...
    std::memcpy(&rx_buffer_ue[offset * payload_length], pkt->data,
                payload_length);
...
void ChannelSim::do_tx_bs(int tid, size_t tag) {
  ...
  cx_fmat fmat_src =
      zeros<cx_fmat>(bscfg->samps_per_symbol(), uecfg->ue_ant_num());
...
void ChannelSim::do_tx_user(int tid, size_t tag) {
  ...
  // apply channel, convert back to complex short to TX
  cx_fmat fmat_src =
      zeros<cx_fmat>(bscfg->samps_per_symbol(), bscfg->bs_ant_num());
  simd_convert_short_to_float(
      src_ptr, reinterpret_cast<float*>(fmat_src.memptr()),
      2 * bscfg->samps_per_symbol() * bscfg->bs_ant_num());
