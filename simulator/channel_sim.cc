...
static void SimdConvertFloatToShort(
    const float* in_buf, short* out_buf, size_t length)
{
    /*
    ...
    , channel_snr_(in_chan_snr)
{
    ...
    ul_data_plus_pilot_symbols_ = bscfg_->ul_data_symbol_num_perframe_
        + bscfg_->pilot_symbol_num_perframe_;
    ...
    message_queue_ = moodycamel::ConcurrentQueue<EventData>(kFrameWnd
        * bscfg_->symbol_num_perframe_
    ...
    for (size_t i = 0; i < worker_thread_num; i++) {
        ...
        if (pthread_create(&task_threads_[i], NULL,
                PthreadFunWrapper<ChannelSim, &ChannelSim::TaskThread>, context)
            != 0) {
        ...
        for (int bulk_count = 0; bulk_count < ret; bulk_count++) {
            ...
            case EventType::kPacketRX: {
                ...
                if (gen_tag_t(event.tags_[0]).tag_type_
                    ...
                    size_t ul_symbol_id
                        = uecfg_->GetUlSymbolIdx(frame_id, symbol_id);
...
void* ChannelSim::TaskThread(int tid)
{
    ...
    PinToCoreWithOffset(ThreadType::kWorker,
        core_offset_ + bs_thread_num_ + 1 + user_thread_num_, tid);
...
void* ChannelSim::UeRxLoop(int tid)
{
    ...
    moodycamel::ProducerToken local_ptok(message_queue_);
    PinToCoreWithOffset(
        ThreadType::kWorkerTXRX, core_offset_ + 1 + bs_thread_num_, tid);
...
void ChannelSim::DoTxBs(int tid, size_t tag)
{
    ...
    size_t symbol_offset = (frame_id % kFrameWnd) * ul_data_plus_pilot_symbols_
        + total_symbol_id;
    ...
    SimdConvertFloatToShort(reinterpret_cast<float*>(fmat_dst.memptr()),
        dst_ptr, 2 * bscfg_->samps_per_symbol_ * bscfg_->bs_ant_num_);
...
void ChannelSim::DoTxUser(int tid, size_t tag)
{
    ...
    SimdConvertFloatToShort(reinterpret_cast<float*>(fmat_dst.memptr()),
        dst_ptr, 2 * bscfg_->samps_per_symbol_ * uecfg_->ue_ant_num_);
