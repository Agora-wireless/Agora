    ...
    , ticks_500(10000 * ticks_per_usec / cfg->symbol_num_perframe)
{
    ...
    ether_addr *parsed_mac = ether_aton(client_mac_addr_str.c_str());
    rt_assert(parsed_mac != NULL, "Invalid client mac address");
    memcpy(&client_mac_addr, parsed_mac, sizeof(ether_addr));

    ret = rte_eth_macaddr_get(portid, &server_mac_addr);
...
void* Sender::worker_thread(int tid)
{
    ...
    while (true) {
        ...
        rte_ether_hdr* eth_hdr
            = rte_pktmbuf_mtod(tx_bufs[0], rte_ether_hdr*);
        eth_hdr->ether_type = rte_be_to_cpu_16(RTE_ETHER_TYPE_IPV4);
        memcpy(eth_hdr->s_addr.addr_bytes, server_mac_addr.addr_bytes, RTE_ETHER_ADDR_LEN);
        memcpy(eth_hdr->d_addr.addr_bytes, client_mac_addr.addr_bytes, RTE_ETHER_ADDR_LEN);

        auto* ip_h = (rte_ipv4_hdr*)((char*)eth_hdr
        ...
        ip_h->total_length = rte_cpu_to_be_16(buffer_length + kPayloadOffset - sizeof(rte_ether_hdr));
        ip_h->packet_id = 0;
        ...
        auto* udp_h
            = (rte_udp_hdr*)((char*)ip_h + sizeof(rte_ipv4_hdr));
        udp_h->src_port = rte_cpu_to_be_16(cfg->ue_server_port + tid);
        udp_h->dst_port = rte_cpu_to_be_16(cfg->bs_port + tid);
        udp_h->dgram_len = rte_cpu_to_be_16(buffer_length + kPayloadOffset - sizeof(rte_ether_hdr) - sizeof(rte_ipv4_hdr));
