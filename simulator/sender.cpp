...
size_t Sender::FindNextSymbol( size_t start_symbol )
{
    ...
    for (next_symbol_id = start_symbol; (next_symbol_id < cfg->frame().NumTotalSyms()); next_symbol_id++)
    {
...
void* Sender::master_thread(int)
{
    ...
    rt_assert(start_symbol != cfg->frame().NumTotalSyms(), "Sender: No valid symbols to transmit");
...
void* Sender::worker_thread(int tid)
{
    ...
    const size_t max_symbol_id = cfg->frame().NumPilotSyms() + cfg->frame().NumULSyms(); //TEMP not sure if this is ok
