...
void* Sender::worker_thread(int tid)
{
    ...
    while (true) {
        ...
        // Update the Packet data to be transmitted based on the IQ sample data.
        pkt->frame_id = tag.frame_id;
        pkt->symbol_id = cfg->getSymbolId(tag.symbol_id);
        pkt->cell_id = 0;
        pkt->ant_id = tag.ant_id;

        // The length of the packet's data array to be sent is as follows:
        // - pre-FFT:  OFDM_FRAME_LEN complex shorts (2 shorts)
        // - post-FFT: OFDM_CA_NUM complex 16-bit floats (so, 2 16-bit floats)
        size_t data_len = cfg->OFDM_FRAME_LEN * sizeof(short) * 2;
        memcpy(pkt->data,
            iq_data_short_[(tag.symbol_id * cfg->BS_ANT_NUM) + tag.ant_id],
            data_len);
        if (cfg->fft_in_rru) {
            run_fft(pkt, fft_inout, mkl_handle);
            data_len = cfg->OFDM_CA_NUM * sizeof(short) * 2;
        }
        ...
        // to a single Millipede server endpoint as normal.
        if (cfg->subcarrier_endpoints.empty()) {
            /*
             * FIXME: (Kevin): I'm pretty sure that we should only be
             * sending `data_len + Packet::kOffsetOfData` bytes
             * instead of `cfg->packet_length` bytes.
             */
        ...
        // the proper per-subcarrier data to each endpoint.
            ...
            for (SubcarrierEndpoint& sc_ep : cfg->subcarrier_endpoints) {
                ...
                // into the beginning of that same packet data buffer.
                // TODO: It'd be best to do this above during the intial memcpy
                // from iq_data_short_ into pkt->data, but that's a more
                // intrusive change that requires restructuring this loop.
                std::memmove(
                    &pkt->data[0], &pkt->data[sc_data_offset], sc_data_len);

                // Remove later. Prevents interleaving of printed log messages.
                std::lock_guard<std::mutex> lock(temp_printing_mutex);

                std::cout << "Sending packet, sc_data_offset " << sc_data_offset
                          << " with sc_data_len " << sc_data_len
                          << " for sc_range " << sc_range.to_string()
                          << std::endl;

                udp_client.send(sc_ep.ip_addr, sc_ep.port,
                    reinterpret_cast<uint8_t*>(socks_pkt_buf),
                    sc_data_len + Packet::kOffsetOfData);
            }
        }
#endif
...
{
    // `pkt->data` currently has `OFDM_FRAME_LEN` short samples.
    // After FFT, we'll remove `OFDM_PREFIX_LEN` initial samples and have
    // `OFDM_CA_NUM` 16-bit float samples, in which each sample is a
    // complex number of two 16-bit floats (hence the `*2`s below.)
