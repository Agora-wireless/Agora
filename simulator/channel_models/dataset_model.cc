...
DatasetModel::DatasetModel( const Config* config, std::string dataset_path ) : ChannelModel( config , ChannelModel::kSelective )
{
        ...
        for( hsize_t frame = 0; frame < frames_num; frame++ )
        {
            ...
            for( hsize_t subcarrier = 0; subcarrier < scs_num; subcarrier++ )
            {
                ...
                for( hsize_t bs = 0; bs < bss_num; bs++ )
                {
                    ...
                    {
                        h_( ue, bs ) = arma::cx_float( real_temp_data[data_index] , im_temp_data[data_index]  );
                        data_index++;
                    }
            ...
            if( kPrintDatasetOutput )
            {
                ...
                std::printf( "Dataset Frame = %ld with %ld Subcarriers loaded successfully \n"
                    , h_matrices_frames.size()
                    , h_subcarrier_matrices.size() );
