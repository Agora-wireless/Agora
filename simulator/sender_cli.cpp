...
DEFINE_string(
    server_mac_addr, "ff:ff:ff:ff:ff:ff", "Millipede server mac address");
...
int main(int argc, char* argv[])
{
    ...
    std::string server_mac_addr_str = FLAGS_server_mac_addr;

    printf("Starting sender\n");
    auto* sender = new Sender(cfg, FLAGS_num_threads, FLAGS_core_offset,
        FLAGS_delay, FLAGS_enable_slow_start, server_mac_addr_str);
    sender->startTX();
    return 0;
}
