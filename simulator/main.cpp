...
int main(int argc, char const* argv[])
{
    ...
    try {
        ...
        if (argc == 5) {
            simulator = new Simulator(cfg, strtol(argv[1], NULL, 10),
                strtol(argv[2], NULL, 10), strtol(argv[3], NULL, 10),
                strtol(argv[4], NULL, 10));
        } else {
            printf("Wrong arguments (requires 3 arguments: 1. number of task "
                   "threads, "
                   "2. number of tx threads, 3. core offset, 4. frame "
                   "duration)\n");
