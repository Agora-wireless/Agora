...
void PhyStats::print_phy_stats()
{
    ...
    for (size_t ue_id = 0; ue_id < cfg->UE_NUM; ue_id++) {
        ...
        std::cout << "UE " << ue_id << " bit errors " << total_bit_errors
...
void PhyStats::update_bit_errors(size_t ue_id, size_t offset, uint8_t tx_byte, uint8_t rx_byte)
{
...
void PhyStats::update_decoded_bits(size_t ue_id, size_t offset, size_t new_bits_num)
{
