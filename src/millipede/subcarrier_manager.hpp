...
public:
        ...
        , dl_ifft_buffer_(dl_ifft_buffer)
    {
        ...
        for (size_t sc_base = 0; sc_base < cfg->OFDM_DATA_NUM;
            ...
            // Worker tids range from `0` to `worker_thread_num`.
        ...
        EventType event_type, size_t frame_id, size_t symbol_id)
    {
        ...
        for (size_t i = 0; i < num_events; i++) {
            // get the subcarrier doer that handles the `sc_id` in `base_tag`.
            auto& matching_doer = subcarrier_doer_for_id(base_tag.sc_id);
            auto& master_to_worker_queue = matching_doer.first;

            try_enqueue_fallback(&master_to_worker_queue.concurrent_q,
