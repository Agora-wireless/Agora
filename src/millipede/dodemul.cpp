...
DoDemul::DoDemul(Config* config, int tid, double freq_ghz,
    ...
    Table<complex_float>& data_buffer, Table<complex_float>& equalizer_buffer,
    ...
    , equalizer_buffer(equalizer_buffer)
...
Event_data DoDemul::launch(size_t tag)
{
    ...
    for (size_t i = 0; i < max_sc_ite; i += kSCsPerCacheline) {
        ...
        for (size_t j = 0; j < kSCsPerCacheline; j++) {
            ...
            const cx_fmat mat_equalizer(
                reinterpret_cast<cx_float*>(cfg->get_equalizer_mat(
                    equalizer_buffer, frame_id, cur_sc_id)),
            ...
            mat_equaled = mat_equalizer * mat_data;
