    ...
    , Demul_task_duration(in_stats_manager->demul_stats_worker.task_duration)
{
    ...
    alloc_buffer_1d(
        &equaled_buffer_temp_transposed, demul_block_size * UE_NUM, 64, 0);
...
void DoDemul::launch(int offset)
{
    ...
    for (int i = 0; i < max_sc_ite / 8; i++) {
        ...
        //         _mm_prefetch((char *)(precoder_ptr + 8 * line_idx),
        //         _MM_HINT_T2);
        //     }
        // }
        ...
        int cur_sc_offset
            = cur_block_id * transpose_block_size * BS_ANT_NUM + sc_inblock_idx;
        float* src_data_ptr
        ...
        for (int j = 0; j < 8; j++) {
...
#endif
            // printf("In doDemul thread %d: frame: %d, subframe: %d,
            // subcarrier: %d, sc_id: %d \n", tid, frame_id,
            // current_data_subframe_id,cur_sc_id, sc_id); cout << "Equaled data
            // sc "<<cur_sc_id<<": "<<mat_equaled.st()<<endl;
            ...
            // int current_data_subframe_id = total_data_subframe_id %
            // data_subframe_num_perframe; printf("In doDemul thread %d: frame:
            // %d, subframe: %d, subcarrier: %d, sc_id: %d \n", tid, frame_id,
            // current_data_subframe_id,cur_sc_id, sc_id); cout<< "Demuled data:
            // "; for (int ue_idx = 0; ue_idx < UE_NUM; ue_idx++) {
            //     // cout<<demod_hard_buffer_[total_data_subframe_id][cur_sc_id
            //     * UE_NUM+ue_idx]<<" "<<endl; cout<<+*(demul_ptr+ue_idx)<<" ";
            // }
            // cout<<endl;
#endif
...
void DoDemul::DemulSingleSC(int offset)
{
    ...
    cx_float* equal_ptr
        = (cx_float*)(&equal_buffer_[total_data_subframe_id][sc_id * UE_NUM]);
    cx_fmat mat_equaled(equal_ptr, UE_NUM, 1, false);
    ...
    uint8_t* demul_ptr
        = (&demod_hard_buffer_[total_data_subframe_id][sc_id * UE_NUM]);
