    ...
    , socket_buffer_(socket_buffer)
{
    ...
    if (cfg->disable_master) {
        spm_buffer = reinterpret_cast<complex_float*>(memalign(
            64, cfg->OFDM_DATA_NUM * cfg->BS_ANT_NUM * sizeof(complex_float)));
    } else {
        spm_buffer = reinterpret_cast<complex_float*>(memalign(
...
void DoDemul::independent_launch(size_t tag)
{
    ...
    for (size_t i = 0; i < max_sc_ite; i += kSCsPerCacheline) {
        for (size_t j = 0; j < cfg->BS_ANT_NUM; j++) {
            float* src = reinterpret_cast<float*>((*socket_buffer_)[j]
            ...
            for (size_t t = 0; t < kSCsPerCacheline; t++) {
                complex_float* dst
                    = spm_buffer + (base_sc_id + i + t) * cfg->BS_ANT_NUM + j;
                *dst = tmp[t];
            }
        }
    }
    ...
    for (size_t i = 0; i < max_sc_ite; i++) {
        ...
        cx_float* data_ptr = reinterpret_cast<cx_float*>(
            &spm_buffer[cur_sc_id * cfg->BS_ANT_NUM]);
        cx_float* ul_zf_ptr = reinterpret_cast<cx_float*>(
            cfg->get_ul_zf_mat(ul_zf_buffer_, frame_id, cur_sc_id));
