...
RadioConfig::RadioConfig(Config* cfg) : _cfg(cfg) {
  ...
  if (_cfg->imbalance_cal_en()) {
    if (_cfg->channel().find('A') != std::string::npos) dciqCalibrationProc(0);
    if (_cfg->channel().find('B') != std::string::npos) dciqCalibrationProc(1);
  }
  ...
  for (size_t i = 0; i < this->_radioNum; i++) {
    ...
    if (_cfg->frame().IsRecCalEnabled()) {
      if (isRefAnt) {
        ...
        std::vector<std::complex<float>> pre(_cfg->ofdm_tx_zero_prefix(), 0);
        std::vector<std::complex<float>> post(_cfg->ofdm_tx_zero_postfix(), 0);
        ...
        recipCalDlPilot.insert(recipCalDlPilot.begin(), pre.begin(), pre.end());
        recipCalDlPilot.insert(recipCalDlPilot.end(), post.begin(), post.end());
        baStn[i]->writeRegisters(
            "TX_RAM_A", 0,
        ...
        if (_cfg->num_channels() == 2) {
          recipCalDlPilot = CommsLib::compose_partial_pilot_sym(
              ...
              kCalibScGroupSize, _cfg->ofdm_ca_num(), _cfg->ofdm_data_num(),
              _cfg->ofdm_data_start(), _cfg->cp_len(), false);
          baStn[i]->writeRegisters(
              "TX_RAM_B", 0,
              Utils::cfloat32_to_uint32(recipCalDlPilot, false, "QI"));
        }
    ...
    if (!kUseUHD) {
      ...
      baStn[i]->activateStream(this->rxStreams[i], SOAPY_SDR_HAS_TIME, 1e9, 0);
      baStn[i]->activateStream(this->txStreams[i], SOAPY_SDR_HAS_TIME, 1e9, 0);
    }
  }
...
int RadioConfig::radioTx(size_t r /*radio id*/, void** buffs, int flags,
  ...
  if (kDebugRadioTX) {
    ...
    long timeoutUs(0);
    int statusFlag = 0;
    int s = baStn[r]->readStreamStatus(this->txStreams[r], chanMask, statusFlag,
                                       frameTime, timeoutUs);
...
int RadioConfig::radioRx(size_t r /*radio id*/, void** buffs,
  ...
  if (r < this->_radioNum) {
    ...
    int ret = baStn[r]->readStream(this->rxStreams[r], buffs,
                                   _cfg->samps_per_symbol(), flags, frameTimeNs,
                                   1000000);
...
void RadioConfig::drain_buffers() {
  ...
  for (size_t i = 0; i < _cfg->num_radios(); i++) {
    RadioConfig::drain_rx_buffer(baStn[i], rxStreams[i], dummybuffs,
                                 _cfg->samps_per_symbol());
  }
}
...
void RadioConfig::drain_rx_buffer(SoapySDR::Device* ibsSdrs,
  ...
  long timeoutUs(0);
  while (r != -1) {
    r = ibsSdrs->readStream(istream, buffs.data(), symSamp, flags, frameTime,
                            timeoutUs);
    i++;
  }
...
void RadioConfig::readSensors() {
  for (size_t i = 0; i < this->_radioNum; i++) {
    ...
    std::cout << "TX0 TEMP  : " << baStn[i]->readSensor(SOAPY_SDR_TX, 0, "TEMP")
              << std::endl;
    std::cout << "TX1 TEMP  : " << baStn[i]->readSensor(SOAPY_SDR_TX, 1, "TEMP")
              << std::endl;
    std::cout << "RX0 TEMP  : " << baStn[i]->readSensor(SOAPY_SDR_RX, 0, "TEMP")
              << std::endl;
    std::cout << "RX1 TEMP  : " << baStn[i]->readSensor(SOAPY_SDR_RX, 1, "TEMP")
              << std::endl;
    std::cout << std::endl;
  }
}
