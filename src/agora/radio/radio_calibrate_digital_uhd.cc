...
auto RadioUHDConfig::TxArrayToRef(
  ...
  for (size_t ant_i = 0; ant_i < tx_antennas; ant_i++) {
    ...
    radios_->Activate(Radio::kActivateWaitTrigger, tx_time, read_samples);
    radios_->Activate(Radio::kActivateWaitTrigger, tx_time, read_samples);

    const auto tx_flags = Radio::TxFlags::kTxWaitTrigger;
    const auto ret_tx =
        radios_->Tx(txbuff.data(), read_samples, tx_flags, tx_time);
    ...
    int rx_status;
    do {
      rx_status = radios_->Rx(rx_buffs, read_samples, rx_flags, rx_time);
...
auto RadioUHDConfig::TxRefToArray(
    const std::vector<std::complex<int16_t>>& tx_vec) {
  //   const size_t num_radios = cfg_->NumRadios();
  const size_t num_radios = 1;
...
bool RadioUHDConfig::FindTimeOffset(
...
void RadioUHDConfig::AdjustDelays(std::vector<int> offset) {
...
void RadioUHDConfig::CalibrateSampleOffset() {
...
bool RadioUHDConfig::InitialCalib() {
  std::cout << "Performing initial calibration... RadioUHDConfig" << std::endl;
  ...
  while ((good_csi_cnt < calib_meas_num_) && (n < (6 * calib_meas_num_))) {
    ...
    for (size_t i = 0; i < r; i++) {
      ...
      for (size_t ch = 0; ch < cfg_->NumChannels(); ch++) {
        ...
        while (retry < max_retries) {
          bad_read = false;
          int ret = radios_->Tx(ch > 0 ? txbuff1.data() : txbuff0.data(),
          ...
          ret = radios_->Rx(rx_buff, read_len, rx_flags, rx_time);
          if (ret < (int)read_len) {
    ...
    if (good_csi == true) {
      ...
      while (retry < max_retries) {
        bad_read = false;
        int ret = radios_->Tx(txbuff0.data(), read_len, tx_flags, tx_time);
        if (ret < (int)read_len) {
          std::cout << "bad write\n";
        }
    ...
    if (good_csi == true) {
      ...
      for (size_t i = 0; i < r; i++) {
        ...
        int ret = radios_->Rx(rx_buff, read_len, rx_flags, rx_time);
        if (ret < (int)read_len) {
          good_csi = false;
