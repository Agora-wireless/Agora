...
static size_t CheckTone(size_t channel, std::vector<Radio*>& rx_devs,
  ...
  for (size_t radio_idx = 0; radio_idx < rx_devs.size(); radio_idx++) {
    ...
    //   const auto samps = dynamic_cast<RadioUHDSdr*>(rx_devs.at(radio_idx))
...
static void AdjustRxGains(size_t channel, std::vector<Radio*>& rx_devs,
  ...
  for (size_t radio_idx = 0u; radio_idx < rx_devs.size(); radio_idx++) {
    ...
    if (tone_level < kToneTargetLevel) {
      ...
      dynamic_cast<RadioUHDSdr*>(rx_devs.at(radio_idx))
...
void RadioUHDConfig::AdjustCalibrationGains(std::vector<Radio*>& rx_devs,
...
void RadioUHDConfig::SetIqBalance(Radio* dev, int direction, size_t channel,
...
void RadioUHDConfig::DciqMinimize(Radio* target_dev, Radio* ref_dev,
  ...
  for (size_t iter = 0; iter < kMaxIterations; iter++) {
    ...
    for (int i = start; i < stop; i += step) {
      ...
      RadioUHDConfig::SetIqBalance(target_dev, direction, channel, gcorr,
                                   iqcorr);
  ...
  RadioUHDConfig::SetIqBalance(target_dev, direction, channel, bestgcorr,
                               bestiqcorr);
...
void RadioUHDConfig::DciqCalibrationProc(size_t channel) {
  std::cout << "****************************************************\n";
  ...
  RadioUHDConfig::AdjustCalibrationGains(all_but_ref_devs, ref_dev, channel,
  ...
  for (size_t r = 0; r < total_radios - 1; r++) {
    RadioUHDConfig::DciqMinimize(all_but_ref_devs.at(r), all_but_ref_devs.at(r),
  ...
  RadioUHDConfig::AdjustCalibrationGains(ref_dev_container, ref_ref_dev,
                                         channel, tone_bb_freq / sample_rate);
  RadioUHDConfig::DciqMinimize(ref_dev, ref_dev, SOAPY_SDR_RX, channel, 0.0,
                               tone_bb_freq / sample_rate);
  ...
  RadioUHDConfig::AdjustCalibrationGains(
      ref_ref_dev_container, ref_dev, channel,
      (tone_bb_freq + tx_tone_bb_freq) / sample_rate);
  RadioUHDConfig::DciqMinimize(ref_dev, ref_ref_dev, SOAPY_SDR_TX, channel,
                               tone_bb_freq / sample_rate,
  ...
  for (size_t r = 0; r < total_radios - 1; r++) {
    ...
    RadioUHDConfig::AdjustCalibrationGains(
        ...
        (tone_bb_freq + tx_tone_bb_freq) / sample_rate);
    RadioUHDConfig::DciqMinimize(radio, ref_dev, SOAPY_SDR_TX, channel,
                                 tone_bb_freq / sample_rate,
                                 tx_tone_bb_freq / sample_rate);
