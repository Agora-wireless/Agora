...
void RadioConfig::dciqCalibrationProc(size_t channel) {
  ...
  RadioConfig::adjustCalibrationGains(allButRefDevs, refDev, channel,
                                      toneBBFreq / sampleRate, true);

  // Minimize Rx DC offset and IQ Imbalance on all receiving radios
  // TODO: Parallelize this loop
  for (size_t r = 0; r < radioSize - 1; r++) {
    RadioConfig::dciqMinimize(allButRefDevs[r], allButRefDevs[r], SOAPY_SDR_RX,
                              channel, 0.0, toneBBFreq / sampleRate);
  }
  ...
  RadioConfig::adjustCalibrationGains(refDevContainer, refRefDev, channel,
                                      toneBBFreq / sampleRate);
  RadioConfig::dciqMinimize(refDev, refDev, SOAPY_SDR_RX, channel, 0.0,
                            toneBBFreq / sampleRate);
  ...
  for (size_t r = 0; r < radioSize - 1; r++) {
    ...
    RadioConfig::adjustCalibrationGains(
        refDevContainer, allButRefDevs[r], channel,
        (toneBBFreq + txToneBBFreq) / sampleRate);
    RadioConfig::dciqMinimize(allButRefDevs[r], refDev, SOAPY_SDR_TX, channel,
                              toneBBFreq / sampleRate,
                              txToneBBFreq / sampleRate);
    allButRefDevs[r]->writeSetting(SOAPY_SDR_TX, channel, "TX_ENB_OVERRIDE",
                                   "false");
    allButRefDevs[r]->writeSetting(SOAPY_SDR_TX, channel, "TSP_TSG_CONST",
                                   "NONE");
...
bool RadioConfig::initial_calib(bool sample_adjust) {
  ...
  for (size_t i = 0; i < R; i++) {
    ...
    for (size_t j = 0; j < R; j++) {
      ...
      ret = baStn[j]->readStream(this->rxStreams[j], rxbuff.data(), read_len,
                                 flags, rxTime, 1000000);
      if (ret < (int)read_len) {
        good_csi = false;
  ...
  for (size_t i = 0; i < R; i++) {
    size_t id = i;
    if (_cfg->external_ref_node() && i == _cfg->ref_ant()) continue;
    if (_cfg->external_ref_node() && i > _cfg->ref_ant()) id = i - 1;
