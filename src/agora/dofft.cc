    ...
    , phy_stats_(in_phy_stats)
{
    ...
    // Aligned for SIMD
    fft_inout_ = static_cast<complex_float*>(
    ...
    rx_samps_tmp_ = static_cast<std::complex<float>*>(
        Agora_memory::PaddedAlignedAlloc(Agora_memory::Alignment_t::kK64Align,
...
static inline void CalibRegressionEstimate(
...
EventData DoFFT::Launch(size_t tag)
{
    ...
    if (cfg_->fft_in_rru_) {
        SimdConvertFloat16ToFloat32(reinterpret_cast<float*>(fft_inout_),
    ...
    if (!cfg_->fft_in_rru_) {
        DftiComputeForward(mkl_handle_,
    ...
    if (sym_type == SymbolType::kPilot) {
        if (kCollectPhyStats) {
            phy_stats_->UpdatePilotSnr(frame_id,
                cfg_->GetPilotSymbolIdx(frame_id, symbol_id), fft_inout_);
        }
    ...
    for (size_t block_idx = 0; block_idx < num_blocks; block_idx++) {
        ...
        for (size_t sc_j = 0; sc_j < kTransposeBlockSize;
            ...
            const complex_float* src
                = &fft_inout_[sc_idx + cfg_->ofdm_data_start_];
            ...
            if (symbol_type == SymbolType::kPilot) {
                ...
                fft_result0 = CommsLib::M256ComplexCf32Mult(
                ...
                fft_result1 = CommsLib::M256ComplexCf32Mult(
...
EventData DoIFFT::Launch(size_t tag)
{
    ...
    if (kMemcpyBeforeIFFT) {
        ...
        if (kUseOutOfPlaceIFFT) {
            ...
            std::memset(
                ifft_in_ptr, 0, sizeof(float) * cfg_->ofdm_data_start_ * 2);
