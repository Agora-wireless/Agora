#include "domatching.hpp"
#include "concurrent_queue_wrapper.hpp"
#include "phy_rate_match.h"

// static constexpr bool kPrintEncodedData = false;

DoRMatching:DoRMatching(Config* in_config, int in_tid,
    Table<int8_t>& in_encoded_data_buffer, Table<int8_t>& in_rmatching_buffer,
    Stats* in_stats_manager)
    : Doer(in_config, in_tid)
    , encoded_data_buffer_(in_encoded_data_buffer)
    , rmatching_buffer_(in_rmatching_buffer)
{
    duration_stat
        = in_stats_manager->get_duration_stat(DoerType::kRMatch, in_tid);

}

DoRMatching::~DoRMatching()
{

}

Event_data DoRMatching::launch(size_t tag)
{
    ...
    if (kDebugPrintInTask) {
        std::printf(
            "In doRMatching thread %d: frame: %zu, symbol: %zu, code block %zu, "
            "ue_id: %zu\n",
    ...
    if (cycles_to_us(duration, cfg->freq_ghz) > 500) {
        std::printf("Thread %d Rate Matching takes %.2f\n", tid,
            cycles_to_us(duration, cfg->freq_ghz));
    }

    return Event_data(EventType::kRMatch, tag);
}

DoDMatching::DoDMatching(Config* in_config, int in_tid,
...
{
    duration_stat
        = in_stats_manager->get_duration_stat(DoerType::kDMatch, in_tid);
...
DoDMatching::~DoDMatching() { free(resp_var_nodes); }

Event_data DoDMatching::launch(size_t tag)
{
    LDPCconfig LDPC_config = cfg->LDPC_config;
    ...
    if (kDebugPrintInTask) {
        std::printf(
            "In doDMatching thread %d: frame: %zu, symbol: %zu, code block: "
            "%zu, ue: %zu\n",
    ...
    return Event_data(EventType::kDMatch, tag);
}
