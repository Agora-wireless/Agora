...
DoPrecode::DoPrecode(
  ...
  duration_stat = in_stats_manager->GetDurationStat(DoerType::kPrecode, in_tid);
  ...
  mkl_jit_status_t status = mkl_jit_create_cgemm(
      &jitter, MKL_COL_MAJOR, MKL_NOTRANS, MKL_NOTRANS, cfg->bs_ant_num(), 1,
      cfg->ue_num(), &alpha, cfg->bs_ant_num(), cfg->ue_num(), &beta,
      cfg->bs_ant_num());
...
Event_data DoPrecode::launch(size_t tag) {
  ...
  if (kUseSpatialLocality) {
    for (size_t i = 0; i < max_sc_ite; i = i + kSCsPerCacheline) {
      ...
      duration_stat->task_count = duration_stat->task_count + kSCsPerCacheline;
  ...
  for (size_t ant_id = 0; ant_id < cfg->bs_ant_num(); ant_id++) {
    int ifft_buffer_offset = ant_id + cfg->bs_ant_num() * total_data_symbol_idx;
    float* ifft_ptr =
...
void DoPrecode::load_input_data(size_t symbol_idx_dl,
  ...
  if ((symbol_idx_dl < cfg->frame().client_dl_pilot_symbols()) ||
    ...
    int8_t* raw_data_ptr =
        &dl_raw_data[total_data_symbol_idx]
                    [sc_id + roundup<64>(cfg->ofdm_data_num()) * user_id];
    data_ptr[user_id] =
...
void DoPrecode::precoding_per_sc(size_t frame_slot, size_t sc_id,
  ...
  auto* data_ptr = reinterpret_cast<cx_float*>(
      modulated_buffer_temp +
      (kUseSpatialLocality ? (sc_id_in_block % kSCsPerCacheline * cfg->ue_num())
                           : 0));
