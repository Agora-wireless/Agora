...
void PacketTXRX::send_beacon(int tid, size_t frame_id) {
  ...
  for (int ant_id = radio_lo; ant_id < radio_hi; ant_id++) {
    ...
    ssize_t r =
        sendto(socket_[ant_id], (char*)udp_pkt_buf.data(), cfg->packet_length(),
...
void PacketTXRX::loop_tx_rx(int tid) {
  ...
  size_t frame_tsc_delta(cfg->GetFrameDurationSec() * 1e9 *
                         measure_rdtsc_freq());
  int prev_frame_id = -1;
...
int PacketTXRX::dequeue_send(int tid) {
  ...
  if (kDebugPrintInTask) {
    std::printf(
        ...
        tid, frame_id, symbol_id, ant_id, gen_tag_t(event.tags[0])._tag, offset,
