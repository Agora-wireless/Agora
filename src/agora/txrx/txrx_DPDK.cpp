...
PacketTXRX::PacketTXRX(Config* cfg, size_t core_offset)
  ...
  DpdkTransport::dpdk_init(kCoreOffset - 1, kSocketThreadNum);
  ...
  for (uint16_t port_id = 0; port_id < cfg_->DpdkNumPorts(); port_id++)
    if (DpdkTransport::nic_init(port_id, mbuf_pool, kSocketThreadNum) != 0)
...
int PacketTXRX::DequeueSend(int tid) {
  ...
  size_t data_symbol_idx_dl = this->cfg_->GetDLSymbolIdx(frame_id, symbol_id);
  size_t offset =
