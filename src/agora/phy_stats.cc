    ...
    : config_(cfg)
{
    ...
    evm_buffer_.Calloc(
        kFrameWnd, cfg->ue_ant_num_, Agora_memory::Alignment_t::kK64Align);

    if (cfg->ul_data_symbol_num_perframe_ > 0) {
        auto *ul_iq_f_ptr
            = reinterpret_cast<cx_float*>(cfg->ul_iq_f_[cfg->ul_pilot_syms_]);
        cx_fmat ul_iq_f_mat(
            ul_iq_f_ptr, cfg->ofdm_ca_num_, cfg->ue_ant_num_, false);
...
void PhyStats::UpdateEvmStats(size_t frame_id, size_t sc_id, cx_fmat eq)
{
    if (this->config_->ul_data_symbol_num_perframe_ > 0) {
        ...
        fmat cur_evm_mat(
            evm_buffer_[frame_id % kFrameWnd], config_->ue_num_, 1, false);
        cur_evm_mat += evm % evm;
    }
}

void PhyStats::UpdateBitErrors(
...
void PhyStats::UpdateUncodedBits(
    size_t ue_id, size_t offset, size_t new_bits_num)
{
