...
#if USE_MKL_JIT
        mkl_jit_cgemm[cfg_->UE_NUM](jitter[cfg_->UE_NUM], (MKL_Complex8*)ul_zf_ptr, (MKL_Complex8*)data_ptr,
            (MKL_Complex8*)equal_ptr);
#else
        const cx_fmat mat_data(data_ptr, cfg_->BS_ANT_NUM, 1, false);
        const cx_fmat mat_ul_zf(ul_zf_ptr, cfg_->UE_NUM, cfg_->BS_ANT_NUM, false);
        mat_equaled = mat_ul_zf * mat_data;
#endif
