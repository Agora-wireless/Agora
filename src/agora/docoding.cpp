...
DoEncode::DoEncode(Config* in_config, int in_tid,
  ...
  duration_stat = in_stats_manager->GetDurationStat(DoerType::kEncode, in_tid);
  parity_buffer = static_cast<int8_t*>(Agora_memory::padded_aligned_alloc(
  ...
  encoded_buffer_temp = static_cast<int8_t*>(Agora_memory::padded_aligned_alloc(
      Agora_memory::Alignment_t::k64Align,
...
DoDecode::DoDecode(
  ...
  duration_stat = in_stats_manager->GetDurationStat(DoerType::kDecode, in_tid);
...
Event_data DoDecode::launch(size_t tag) {
  ...
  if ((kEnableMac == false) && (kPrintPhyStats == true) &&
    ...
    phy_stats->update_decoded_bits(ue_id, symbol_offset,
                                   cfg->num_bytes_per_cb() * 8);
    ...
    for (size_t i = 0; i < cfg->num_bytes_per_cb(); i++) {
      ...
      uint8_t tx_byte = static_cast<uint8_t>(
          cfg->GetInfoBits(cfg->ul_bits(), symbol_idx_ul, ue_id, cur_cb_id)[i]);
      phy_stats->update_bit_errors(ue_id, symbol_offset, tx_byte, rx_byte);
      if (rx_byte != tx_byte) {
        block_error++;
      }
    }
