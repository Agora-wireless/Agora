    ...
    SoapySDR::Device* txDev, size_t channel, double fftBin, bool plot)
{
    ...
    for (size_t r = 0; r < rx_devs_size; r++) {
        ...
        rxDevs[r]->setGain(SOAPY_SDR_RX, channel, next_gain_stage,
    ...
    for (size_t r = 0; r < rx_devs_size; r++) {
        ...
        if (kIQImbalancePlot && plot) {
            ...
            plt::legend();
            plt::save("rx" + std::to_string(rx_devs_size) + "_"
                + std::to_string(r) + "_ch" + std::to_string(channel) + ".png");
        }
    }

    std::cout << rx_devs_size - remaining_radios
              << " radios reached target level" << std::endl;
}
    ...
    double rxCenterTone, double txCenterTone)
{
    ...
    // measure initial
    {
        ...
        std::cout << "dciqMinimize initial: dcLvl=" << meas_dc_level
                  ...
                  << " dB, toneLevel=" << desired_tone_level << "dB"
                  << std::endl;
    }
    ...
    for (size_t iter = 0; iter < 4; iter++) {
        ...
        if (iter > 1) // narrow in for the final iteration set
        {
            const int center = int(
                (((iter % 2) == 0) ? best_dc_corr.imag() : best_dc_corr.real())
                * fixed_scale);
        ...
        for (int i = start; i < stop; i += step) {
            ...
            const auto dc_corr = ((iter % 2) == 0)
                ? std::complex<double>(
                    best_dc_corr.real(), double(i) / fixed_scale)
                : std::complex<double>(
                    double(i) / fixed_scale, best_dc_corr.imag());
            ...
            const auto samps = SnoopSamples(refDev, channel, n);
            const auto meas_dc_level = CommsLib::MeasureTone(
                samps, win, window_gain, rxCenterTone, n);
...
void RadioConfig::DciqCalibrationProc(size_t channel)
{
    ...
    std::cout << "   DC Offset and IQ Imbalance Calibration: Ch " << channel
              << std::endl;
    std::cout << "****************************************************\n";
    double sample_rate = cfg_->rate_;
    double center_rf_freq = cfg_->radio_rf_freq_;
    double tone_bb_freq = sample_rate / 7;
    size_t radio_size = cfg_->n_radios_;

    size_t reference_radio = cfg_->ref_ant_ / cfg_->n_channels_;
    ...
    ref_dev->setFrequency(
        SOAPY_SDR_TX, channel, "RF", center_rf_freq + tone_bb_freq);
    ...
    RadioConfig::AdjustCalibrationGains(
        all_but_ref_devs, ref_dev, channel, tone_bb_freq / sample_rate, true);
    ...
    for (size_t r = 0; r < radio_size - 1; r++) {
        RadioConfig::DciqMinimize(all_but_ref_devs[r], all_but_ref_devs[r],
            SOAPY_SDR_RX, channel, 0.0, tone_bb_freq / sample_rate);
    }
    ...
    RadioConfig::AdjustCalibrationGains(
        ref_dev_container, ref_ref_dev, channel, tone_bb_freq / sample_rate);
    ...
    ref_ref_dev->setFrequency(SOAPY_SDR_RX, channel, "BB",
    ...
    RadioConfig::AdjustCalibrationGains(ref_ref_dev_container, ref_dev, channel,
        (tone_bb_freq + tx_tone_bb_freq) / sample_rate);
    RadioConfig::DciqMinimize(ref_dev, ref_ref_dev, SOAPY_SDR_TX, channel,
        tone_bb_freq / sample_rate, tx_tone_bb_freq / sample_rate);
    ...
    // centerRfFreq-toneBBFreq at true center?
    for (size_t r = 0; r < radio_size - 1; r++) {
        all_but_ref_devs[r]->setFrequency(
            SOAPY_SDR_TX, channel, "RF", center_rf_freq);
        all_but_ref_devs[r]->setFrequency(
            SOAPY_SDR_TX, channel, "BB", tx_tone_bb_freq);
        all_but_ref_devs[r]->writeSetting(
            SOAPY_SDR_TX, channel, "TSP_TSG_CONST", std::to_string(1 << 14));
        all_but_ref_devs[r]->writeSetting(
        ...
        RadioConfig::AdjustCalibrationGains(ref_dev_container,
            all_but_ref_devs[r], channel,
            (tone_bb_freq + tx_tone_bb_freq) / sample_rate);
        RadioConfig::DciqMinimize(all_but_ref_devs[r], ref_dev, SOAPY_SDR_TX,
            channel, tone_bb_freq / sample_rate, tx_tone_bb_freq / sample_rate);
        all_but_ref_devs[r]->writeSetting(
            SOAPY_SDR_TX, channel, "TX_ENB_OVERRIDE", "false");
        all_but_ref_devs[r]->writeSetting(
...
void RadioConfig::AdjustDelays(std::vector<int> offset)
{
    ...
    size_t ref_offset = cfg_->ref_ant_ == 0 ? 1 : 0;
...
bool RadioConfig::InitialCalib(bool sample_adjust)
{
    ...
    for (size_t i = 0; i < r; i++) {
        ba_stn_[i]->setGain(SOAPY_SDR_TX, ch, "PAD",
            ch != 0u ? cfg_->calib_tx_gain_b_ : cfg_->calib_tx_gain_a_);
    ...
    while (good_csi_cnt < calib_meas_num_ && n < 2 * calib_meas_num_) {
        ...
        for (size_t i = 0; i < r; i++) {
            ...
            int ret
                = ba_stn_[i]->writeStream(this->tx_streams_[i], txbuff0.data(),
                    cfg_->pilot_ci16_.size(), tx_flags, tx_time, 1000000);
            if (ret < (int)read_len) {
                std::cout << "bad write\n";
            }
            for (size_t j = 0; j < r; j++) {
                ...
                ret = ba_stn_[j]->activateStream(
                    this->rx_streams_[j], rx_flags, rx_time, read_len);
            }
            ...
            for (size_t j = 0; j < r; j++) {
                ...
                ret = ba_stn_[j]->readStream(this->rx_streams_[j],
                    rxbuff.data(), read_len, flags, rx_time, 1000000);
                if (ret < (int)read_len) {
                    good_csi = false;
        ...
        for (size_t i = 0; i < r; i++) {
            ...
            int flags = 0;
            int ret = ba_stn_[i]->activateStream(
                this->rx_streams_[i], rx_flags, rx_time, read_len);
            std::vector<void*> rxbuff(2);
        ...
        for (size_t i = 0; i < r; i++) {
            ...
            std::transform(buff[cfg_->ref_ant_ * r + i].begin(),
                buff[cfg_->ref_ant_ * r + i].end(), up[i].begin(),
                [](std::complex<int16_t> ci) {
                    return std::complex<float>(
                        ci.real() / 32768.0, ci.imag() / 32768.0);
                });
            std::transform(buff[i * r + cfg_->ref_ant_].begin(),
                buff[i * r + cfg_->ref_ant_].end(), dn[i].begin(),
                [](std::complex<int16_t> ci) {
                    return std::complex<float>(
                        ci.real() / 32768.0, ci.imag() / 32768.0);
                });
            ...
            size_t peak_up
                = CommsLib::FindPilotSeq(up[i], cfg_->pilot_cf32_, seq_len);
            size_t peak_dn
                = CommsLib::FindPilotSeq(dn[i], cfg_->pilot_cf32_, seq_len);
            start_up[i]
                = peak_up < seq_len ? 0 : peak_up - seq_len + cfg_->cp_len_;
            start_dn[i]
                = peak_dn < seq_len ? 0 : peak_dn - seq_len + cfg_->cp_len_;
            ...
            float sig_up = 0;
            float noise_up = 0;
            for (size_t q = 0; q < cfg_->ofdm_ca_num_; q++) {
            ...
            for (size_t q = 0; q < cfg_->ofdm_ca_num_; q++) {
                ...
                noise_dn += std::pow(
                    std::abs(noise[cfg_->ref_ant_][q + start_dn[i]]), 2);
            }
            ...
            if (kReciprocalCalibPlot) {
                ...
                // plt::xlim(0, read_len);
                plt::ylim(-1, 1);
                plt::title("ant " + std::to_string(cfg_->ref_ant_)
        ...
        for (size_t i = 0; i < r; i++) {
            size_t id = i;
            if (cfg_->external_ref_node_ && i == cfg_->ref_ant_) {
                continue;
            }
            if (cfg_->external_ref_node_ && i > cfg_->ref_ant_) {
                id = i - 1;
            ...
            if (kVerboseCalibration) { // print time-domain data
                ...
                for (size_t j = 0; j < read_len; j++) {
                    std::cout << buff[cfg_->ref_ant_ * r + i][j].real()
                              << "+1j*"
                              << buff[cfg_->ref_ant_ * r + i][j].imag() << " ";
                }
                std::cout << "];" << std::endl;
                std::cout << "dn_t" << id << " = [";
                for (size_t j = 0; j < read_len; j++) {
                    std::cout << buff[i * r + cfg_->ref_ant_][j].real()
                              << "+1j*"
                              << buff[i * r + cfg_->ref_ant_][j].imag() << " ";
                }
                std::cout << "];" << std::endl;
            }
            // computing initial calib vectors
            auto first_up = up[i].begin() + start_up[i];
            auto last_up = up[i].begin() + start_up[i] + cfg_->ofdm_ca_num_;
            std::vector<std::complex<float>> up_ofdm(first_up, last_up);
            assert(up_ofdm.size() == cfg_->ofdm_ca_num_);

            auto first_dn = dn[i].begin() + start_dn[i];
            auto last_dn = dn[i].begin() + start_dn[i] + cfg_->ofdm_ca_num_;
            std::vector<std::complex<float>> dn_ofdm(first_dn, last_dn);
            assert(dn_ofdm.size() == cfg_->ofdm_ca_num_);

            auto dn_f = CommsLib::FFT(dn_ofdm, cfg_->ofdm_ca_num_);
            auto up_f = CommsLib::FFT(up_ofdm, cfg_->ofdm_ca_num_);
            arma::cx_fvec dn_vec(reinterpret_cast<arma::cx_float*>(
                                     &dn_f[cfg_->ofdm_data_start_]),
                cfg_->ofdm_data_num_, false);
            arma::cx_fvec calib_dl_vec(
                reinterpret_cast<arma::cx_float*>(
                    &init_calib_dl_[good_csi_cnt][id * cfg_->ofdm_data_num_]),
                cfg_->ofdm_data_num_, false);
            calib_dl_vec = dn_vec;

            arma::cx_fvec up_vec(reinterpret_cast<arma::cx_float*>(
                                     &up_f[cfg_->ofdm_data_start_]),
                cfg_->ofdm_data_num_, false);
            arma::cx_fvec calib_ul_vec(
                reinterpret_cast<arma::cx_float*>(
                    &init_calib_ul_[good_csi_cnt][id * cfg_->ofdm_data_num_]),
                cfg_->ofdm_data_num_, false);
            calib_ul_vec = up_vec;
