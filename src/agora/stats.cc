...
double Stats::MeasureLastFrameLatency() {
  const size_t frame_id = this->last_frame_id_;
  // Latency of symbol_id is sent to RP for frame_id
  const size_t symbol_id = (config_->Frame().NumTotalSyms() - 1);
  ...
  size_t master_min_tsc_symbolrx =
      MasterGetTscSymbol(TsType::kSymbolRX, frame_id, symbol_id, 0);
  size_t master_min_tsc_symbolrx_idx = 0;
  for (size_t l = 1; l < config_->BsAntNum(); l++) {
    if (MasterGetTscSymbol(TsType::kSymbolRX, frame_id, symbol_id, l) <
        master_min_tsc_symbolrx) {
      master_min_tsc_symbolrx =
          MasterGetTscSymbol(TsType::kSymbolRX, frame_id, symbol_id, l);
      master_min_tsc_symbolrx_idx = l;
    }
  }
  size_t master_max_tsc_decode =
      MasterGetTscSymbol(TsType::kDecodeDone, frame_id, symbol_id, 0);
  size_t master_max_tsc_decode_idx = 0;
  for (size_t l = 1; l < config_->SpatialStreamsNum(); l++) {
    if (MasterGetTscSymbol(TsType::kDecodeDone, frame_id, symbol_id, l) >
        master_max_tsc_decode) {
      master_max_tsc_decode =
          MasterGetTscSymbol(TsType::kDecodeDone, frame_id, symbol_id, l);
      master_max_tsc_decode_idx = l;
    }
  }

  const double processing_started =
      MasterGetUsSymbolFromRef(TsType::kSymbolRX, frame_id, symbol_id,
                               master_min_tsc_symbolrx_idx, ref_tsc);
  const double decoding_done =
      MasterGetUsSymbolFromRef(TsType::kDecodeDone, frame_id, symbol_id,
                               master_max_tsc_decode_idx, ref_tsc);
...
void Stats::SaveToFile() {
  ...
  if (config_->DynamicCoreAlloc()) {
    const size_t snr_db = static_cast<size_t>(
        10 * std::log10((float)(1.0F) /
                        (config_->NoiseLevel() * config_->NoiseLevel())));
    ...
    const size_t frame_duration = (config_->Frame().NumTotalSyms() *
                                   config_->SampsPerSymbol() * 1000000ul) /
                                  config_->Rate();

    const size_t ticks_per_usec = (this->freq_ghz_ * 1e3);
    const size_t ticks_per_symbol =
        ((frame_duration * ticks_per_usec) / config_->Frame().NumTotalSyms());
    size_t ul_data_symbol_start =
        config_->Frame().NumTotalSyms() - config_->Frame().NumULSyms();

    if (config_->Frame().NumULSyms() > 0) {
      // Print the header
      std::fprintf(fp_debug_symbol,
                   "Frame, Symbol, "
                   "Pilot RX by socket threads (= reference tsc), "
                   "Pilot RX by socket threads (= reference time), "
                   "kSymbolRX (tsc), kDecodeDone (tsc), (kDecodeDone (tsc) - "
                   "kSymbolRX (tsc)), "
                   "kSymbolRX (time), kDecodeDone (time), (kDecodeDone (time) "
                   "- kSymbolRX (time)), doer_sum (time)\n");
      ...
      for (size_t frame = 0; frame < total_stat_frames; frame++) {
        ...
        for (size_t k = ul_data_symbol_start;
             k < config_->Frame().NumTotalSyms(); k++) {
          size_t master_min_tsc_symbolrx =
              MasterGetTscSymbol(TsType::kSymbolRX, i, k, 0);
          size_t master_min_tsc_symbolrx_idx = 0;
          for (size_t l = 1; l < config_->BsAntNum(); l++) {
            if (MasterGetTscSymbol(TsType::kSymbolRX, i, k, l) <
                master_min_tsc_symbolrx) {
              master_min_tsc_symbolrx =
                  MasterGetTscSymbol(TsType::kSymbolRX, i, k, l);
              master_min_tsc_symbolrx_idx = l;
            }
          }
          size_t master_max_tsc_decode =
              MasterGetTscSymbol(TsType::kDecodeDone, i, k, 0);
          size_t master_max_tsc_decode_idx = 0;
          for (size_t l = 1; l < config_->SpatialStreamsNum(); l++) {
            if (MasterGetTscSymbol(TsType::kDecodeDone, i, k, l) >
                master_max_tsc_decode) {
              master_max_tsc_decode =
                  MasterGetTscSymbol(TsType::kDecodeDone, i, k, l);
              master_max_tsc_decode_idx = l;
            }
          }
          std::fprintf(
              fp_debug_symbol,
              "%zu %zu %zu %.3f %zu %zu %zu %.3f %.3f %.3f %.3f\n", i, k,
              ref_tsc,
              GetTime::CyclesToUs(ref_tsc - this->creation_tsc_,
                                  this->freq_ghz_),
              master_min_tsc_symbolrx, master_max_tsc_decode,
              (master_max_tsc_decode - master_min_tsc_symbolrx),
              MasterGetUsSymbolFromRef(TsType::kSymbolRX, i, k,
                                       master_min_tsc_symbolrx_idx, ref_tsc),
              MasterGetUsSymbolFromRef(TsType::kDecodeDone, i, k,
                                       master_max_tsc_decode_idx, ref_tsc),
              (MasterGetUsSymbolFromRef(TsType::kDecodeDone, i, k,
                                        master_max_tsc_decode_idx, ref_tsc) -
               MasterGetUsSymbolFromRef(TsType::kSymbolRX, i, k,
