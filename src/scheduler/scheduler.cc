...
Scheduler::Scheduler(Config* const cfg, PtrGrid<kFrameWnd, kMaxUEs, complex_float>& csi_buffers) 
    : cfg_(cfg), csi_buffers_(csi_buffers) {
  // init with Agora
  select_num = std::min(FLAGS_select_num, cfg->UeNum());
  this->selection_.Alloc(kFrameWnd, cfg_->Frame().NumDLSyms(), select_num*cfg_->UeNum());
  last_ue = 0;
  AllocBuffer1d(&data_queue_, cfg_->UeAntNum(), Agora_memory::Alignment_t::kAlign32, 1);
...
void Scheduler::Launch(size_t frame_id) {
  ...
  // for (size_t symbol_id = 0; symbol_id < cfg_->Frame().NumDLSyms(); ++symbol_id) {
  for (size_t symbol_id = 0; !symbol_id; ++symbol_id) {
    unsigned int *selection = selection_[frame_id][symbol_id];
    memset(selection, 0, sizeof(unsigned int)*select_num*cfg_->UeNum());
    // memset(selection, 0, sizeof(unsigned int)*cfg_->UeNum()*cfg_->UeNum());
    for (size_t i = 0; i < select_num; ++i) {
      selection[last_ue*select_num + i] = 1;
      // selection[i*cfg_->UeNum() + last_ue] = 1;
      last_ue = (last_ue + 1)%cfg_->UeNum();
    }
  }
  update.store(true);
...
void Scheduler::PrintQueue() {
    ...
    for (size_t ue_id = 0; ue_id < cfg_->UeNum(); ++ue_id) {
...
void Scheduler::PrintSelect(size_t frame_id, size_t symbol_id) {
  unsigned int *selection = selection_[frame_id][symbol_id];
  arma::Mat<arma::u32> selection_mat(reinterpret_cast<arma::u32*>(selection), select_num, cfg_->UeNum(), false);
...
void Scheduler::UpdateQueue(size_t frame_id) {
  ...
  auto dqueue = arma::Row<arma::u32>(reinterpret_cast<arma::u32*>(data_queue_), cfg_->UeNum(), false);
