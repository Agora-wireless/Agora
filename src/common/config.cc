...
Config::Config(const std::string& jsonfile)
  ...
  if (kUseRemote) {
    remote_ldpc_ip_addr_ = tdd_conf.value("remote_ldpc_ip_addr", "127.0.0.1");
    remote_ldpc_completion_port_ =
        tdd_conf.value("remote_ldpc_completion_port", 31850);
    remote_ldpc_base_port_ = tdd_conf.value("remote_ldpc_base_port", 31851);
    remote_ldpc_num_threads_ =
        tdd_conf.value("remote_ldpc_num_threads", worker_thread_num_);
    remote_ldpc_core_offset_ = tdd_conf.value("remote_ldpc_core_offset", 0);
#if defined(USE_DPDK)
    remote_ldpc_mac_addr = tdd_conf.value("remote_ldpc_mac_addr", "");
    local_mac_addr = tdd_conf.value("local_mac_addr", "");
#endif  // USE_DPDK
  }
...
size_t Config::GetLdpcInputOffset(size_t cb_id) const {
  size_t cur_cb_id = cb_id % ldpc_config_.NumBlocksInSymbol();
  size_t ue_id = cb_id / ldpc_config_.NumBlocksInSymbol();
  return ofdm_data_num_ * ue_id + ldpc_config_.NumCbCodewLen() * cur_cb_id;
}

size_t Config::GetLdpcOutputOffset(size_t cb_id) const {
  size_t cur_cb_id = cb_id % ldpc_config_.NumBlocksInSymbol();
  size_t ue_id = cb_id / ldpc_config_.NumBlocksInSymbol();
  size_t cbLenBytes = BitsToBytes(ldpc_config_.NumCbLen());
  return cbLenBytes * ldpc_config_.NumBlocksInSymbol() * ue_id +
