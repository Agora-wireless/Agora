    ...
    : freq_ghz_(MeasureRdtscFreq())
{
    ...
    auto gain_adj_json_a
        = tdd_conf.value("client_gain_adjust_a", json::array());
    ...
    auto gain_adj_json_b
        = tdd_conf.value("client_gain_adjust_b", json::array());
    ...
    ofdm_rx_zero_prefix_client_
        = tdd_conf.value("ofdm_rx_zero_prefix_client", 0);
    ...
    ofdm_data_start_ = tdd_conf.value(
        "ofdm_data_start", (ofdm_ca_num_ - ofdm_data_num_) / 2);
    ...
    if (tdd_conf.find("frames") == tdd_conf.end()) {
        ...
        pilot_symbol_num_perframe_ = tdd_conf.value(
            "pilot_num", freq_orthogonal_pilot_ ? 1 : ue_ant_num_);
        ul_data_symbol_num_perframe_ = tdd_conf.value("ul_symbol_num_perframe",
            downlink_mode_
                ? 0
        ...
        if (downlink_mode_) {
            ...
            for (size_t s = 1 + pilot_symbol_num_perframe_;
    ...
    recip_cal_en_
        = (!ul_cal_symbols_[0].empty() and !dl_cal_symbols_[0].empty());
    ...
    data_symbol_num_perframe_ = symbol_num_perframe_
        - pilot_symbol_num_perframe_ - beacon_symbol_num_perframe_;
    ...
    if (is_ue_ and !freq_orthogonal_pilot_
        and ue_ant_num_ != pilot_symbol_num_perframe_) {
        RtAssert(false, "Number of pilot symbols doesn't match number of UEs");
    }
    if (!is_ue_ and !freq_orthogonal_pilot_
        and tdd_conf.find("ue_num") == tdd_conf.end()) {
    ...
    RtAssert(demul_block_size_ % kSCsPerCacheline == 0,
        ...
        "cacheline");
    RtAssert(demul_block_size_ % kTransposeBlockSize == 0,
    ...
    zf_block_size_ = freq_orthogonal_pilot_
        ? ue_ant_num_
    ...
    std::printf(
        ...
        ul_data_symbol_num_perframe_, dl_data_symbol_num_perframe_,
        ofdm_ca_num_, ofdm_data_num_, modulation_.c_str(),
...
void Config::GenData()
{
    if (kUseArgos || kUseUHD) {
        ...
        beacon_ci16_.insert(
            beacon_ci16_.begin(), pre_beacon.begin(), pre_beacon.end());
        beacon_ci16_.insert(
            beacon_ci16_.end(), post_beacon.begin(), post_beacon.end());
    }
    ...
    pilots_ = (complex_float*)aligned_alloc(
        64, ofdm_data_num_ * sizeof(complex_float));
    ...
    AllocBuffer1d(
        &pilot_ifft, ofdm_ca_num_, Agora_memory::Alignment_t::kK64Align, 1);
    ...
    ue_specific_pilot_.Malloc(
        ue_ant_num_, ofdm_data_num_, Agora_memory::Alignment_t::kK64Align);
    ue_specific_pilot_t_.Calloc(
        ue_ant_num_, samps_per_symbol_, Agora_memory::Alignment_t::kK64Align);
    Table<complex_float> ue_pilot_ifft;
    ue_pilot_ifft.Calloc(
        ue_ant_num_, ofdm_ca_num_, Agora_memory::Alignment_t::kK64Align);
    ...
    dl_iq_f_.Calloc(dl_data_symbol_num_perframe_, ofdm_ca_num_ * ue_ant_num_,
        Agora_memory::Alignment_t::kK64Align);
    dl_iq_t_.Calloc(dl_data_symbol_num_perframe_,
        samps_per_symbol_ * ue_ant_num_, Agora_memory::Alignment_t::kK64Align);
    ...
    ul_iq_f_.Calloc(ul_data_symbol_num_perframe_, ofdm_ca_num_ * ue_ant_num_,
        Agora_memory::Alignment_t::kK64Align);
    ul_iq_t_.Calloc(ul_data_symbol_num_perframe_,
        samps_per_symbol_ * ue_ant_num_, Agora_memory::Alignment_t::kK64Align);
    ...
    // Encode uplink bits
    ul_encoded_bits_.Malloc(
        ul_data_symbol_num_perframe_ * num_blocks_per_symbol,
        encoded_bytes_per_block, Agora_memory::Alignment_t::kK64Align);
    ul_mod_input_.Calloc(ul_data_symbol_num_perframe_,
        ofdm_data_num_ * ue_ant_num_, Agora_memory::Alignment_t::kK32Align);
    int8_t* temp_parity_buffer = new int8_t[LdpcEncodingParityBufSize(
        ldpc_config_.bg_, ldpc_config_.zc_)];
        ...
        for (size_t j = 0; j < ue_ant_num_; j++) {
            for (size_t k = 0; k < ldpc_config_.nblocks_in_symbol_; k++) {
                ...
                LdpcEncodeHelper(ldpc_config_.bg_, ldpc_config_.zc_,
                    ldpc_config_.n_rows_, coded_bits_ptr, temp_parity_buffer,
                    bits_ptr);
    ...
    for (size_t i = 0; i < ul_data_symbol_num_perframe_; i++) {
        for (size_t u = 0; u < ue_ant_num_; u++) {
            ...
            for (size_t j = ofdm_data_start_; j < ofdm_data_stop_; j++) {
                ...
                size_t s = p + k;
                ul_iq_f_[i][q + j]
                    = ModSingleUint8(ul_mod_input_[i][s], mod_table_);
        ...
        for (size_t u = 0; u < ue_ant_num_; u++) {
            ...
            for (size_t j = ofdm_data_start_; j < ofdm_data_stop_; j++) {
                ...
                if (k % ofdm_pilot_spacing_ != 0) {
                    dl_iq_f_[i][q + j]
                        = ModSingleUint8(dl_mod_input_[i][s], mod_table_);
                } else {
    ...
    float max_val = CommsLib::FindMaxAbs(
    ...
    cur_max_val
        = CommsLib::FindMaxAbs(ue_pilot_ifft, ue_ant_num_, ofdm_ca_num_);
    if (cur_max_val > max_val) {
        max_val = cur_max_val;
    }
    ...
    for (size_t i = 0; i < ue_ant_num_; i++) {
        CommsLib::Ifft2tx(ue_pilot_ifft[i], ue_specific_pilot_t_[i],
    ...
    for (size_t i = 0; i < ofdm_ca_num_; i++) {
        pilot_cf32_.push_back(std::complex<float>(
...
size_t Config::GetPilotSymbolIdx(size_t frame_id, size_t symbol_id) const
{
    size_t fid = frame_id % frames_.size();
    const auto it = find(
        pilot_symbols_[fid].begin(), pilot_symbols_[fid].end(), symbol_id);
