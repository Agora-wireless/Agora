...
Config::Config(std::string jsonfile)
{
    ...
    server_addr_to_mac = tddConf.value("server_addr_to_mac", "127.0.0.1");
    bs_port = tddConf.value("bs_port", 8000);
    ue_client_port = tddConf.value("ue_client_port", 7000);
    ue_server_port = tddConf.value("ue_server_port", 6000);
    mac_client_port = tddConf.value("mac_client_port", 5000);
    mac_server_port = tddConf.value("mac_client_port", 4000);
...
void Config::genData()
{
    ...
    if (kUseLDPC) {
        ...
        dl_encoded_bits.malloc(
            dl_data_symbol_num_perframe * num_blocks_per_symbol,
            encoded_bytes_per_block, 64);
