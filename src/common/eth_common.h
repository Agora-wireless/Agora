...
struct eth_routing_info_t {
    ...
    {
        std::ostringstream ret;
        ret << "[MAC " << mac_to_string(mac) << ", IP "
            << ipv4_to_string(ipv4_addr) << ", UDP port "
...
struct ipv4_hdr_t {
    ...
    std::string to_string() const
    {
        ...
        ret << "[IPv4: ihl " << std::to_string(ihl) << ", version "
            ...
            << std::to_string(ntohs(frag_off)) << ", ttl "
            << std::to_string(ttl) << ", protocol " << std::to_string(protocol)
            << ", check " << std::to_string(check) << ", src IP "
            << ipv4_to_string(src_ip) << ", dst IP " << ipv4_to_string(dst_ip)
            << "]";
        return ret.str();
    }
} __attribute__((packed));
...
struct udp_hdr_t {
    ...
    {
        std::ostringstream ret;
        ret << "[UDP: src_port " << std::to_string(ntohs(src_port))
            << ", dst_port " << std::to_string(ntohs(dst_port)) << ", len "
...
static inline void gen_ipv4_header(
    ipv4_hdr_t* ipv4_hdr, uint32_t src_ip, uint32_t dst_ip, uint16_t data_size)
{
    ipv4_hdr->version = 4;
    ipv4_hdr->ihl = 5;
...
static inline uint32_t get_interface_ipv4_addr(std::string interface)
{
    ...
    rt_assert(
        ipv4_addr != 0, std::string("Failed to find interface ") + interface);
