...
void MatLogger::SaveMatBuf() {
  ...
  for (size_t frame_idx = 0; frame_idx < kFrames; frame_idx++) {
    for (size_t sym_idx = 0; sym_idx < kSymbols; sym_idx++) {
      for (size_t sc_idx = 0; sc_idx < kSCs; sc_idx++) {
        for (size_t i = 0; i < kBSAnts; i++) {
          for (size_t j = 0; j < kUEAnts; j++) {
            const arma::cx_float& cx = mat_buffer_.at(frame_idx).at(sym_idx).at(sc_idx)(i, j);
            Write(frame_idx + kFrameStart, sym_idx, sc_idx, i, j, cx.real(), cx.imag());
          }  // end kUEAnts
        }    // end kBSAnts
      }      // end kSCs
    }        // end kSymbols
