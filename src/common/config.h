...
class Config {
 public:
  ...
  inline size_t DemodBlockSize() const { return this->demod_block_size_; }

  inline size_t DemulEventsPerSymbol() const {
    return this->demul_events_per_symbol_;
  }
  inline size_t DemodEventsPerSymbol() const {
    return this->demod_events_per_symbol_;
  }
  ...
  inline int8_t* GetModBitsBuf(Table<int8_t>& mod_bits_buffer, Direction dir,
    ...
    return &mod_bits_buffer[total_data_symbol_id]
                           [Roundup<64>(GetOFDMDataNum()) * ue_id + sc_id];
  }
  ...
  inline size_t GetOFDMSymbolIndex(size_t data_id) const {
    return data_symbol_id_.at(data_id);
  }
 ...
 private:
  ...
  std::vector<size_t> data_symbol_id_;
  ...
  size_t demod_block_size_;
  size_t demod_events_per_symbol_;  // Derived from demul_block_size
