...
public:
    ...
    inline uint8_t* GetDecodeBuf(Table<uint8_t>& decoded_buffer,
        size_t frame_id, size_t symbol_id, size_t ue_id, size_t cb_id) const
    {
        size_t total_data_symbol_id
            = GetTotalDataSymbolIdxUl(frame_id, symbol_id);
    ...
    {
        return &info_bits[symbol_id][Roundup<64>(num_bytes_per_cb_)
            * (ldpc_config_.nblocks_in_symbol_ * ue_id + cb_id)];
    }
