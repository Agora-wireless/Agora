...
FrameStats::FrameStats(std::string new_frame_id, size_t ul, size_t dl) : FrameStats(new_frame_id)
{
...
size_t FrameStats::NumDLSyms( void ) const
{
    return this->dl_symbols_.size();
}

size_t FrameStats::NumULSyms( void ) const
{
...
{
    return ( (this->ul_cal_symbols_.size() > 0) && (this->dl_cal_symbols_.size() > 0) );
}

size_t FrameStats::NumDataSyms( void ) const
{
    return (this->NumTotalSyms() - (this->pilot_symbols_.size() + this->beacon_symbols_.size()));
}
...
size_t FrameStats::GetSymbolIdx ( const std::vector<size_t>& search_vector, size_t symbol_number )
{
    /* Todo Optimize this, probably with a precomputed table -- assumes sorted list*/
    const auto [start, finish] = std::equal_range(search_vector.begin(), search_vector.end(), symbol_number);
