...
public:
    ...
    void malloc(size_t dim1, size_t dim2, Agora_memory::Alignment_t alignment)
    {
        ...
        this->data_ = static_cast<T*>(Agora_memory::padded_aligned_alloc(alignment, alloc_size));
    }
    ...
    // -1.0 and 1.0
    void rand_alloc_float(size_t dim1, size_t dim2, Agora_memory::Alignment_t alignment)
    {
    ...
    // between -1.0 and 1.0
    void rand_alloc_cx_float(size_t dim1, size_t dim2, Agora_memory::Alignment_t alignment)
    {
        std::default_random_engine generator;
        std::uniform_real_distribution<float> distribution(-1.0, 1.0);
...
static void alloc_buffer_1d(T** buffer, U dim, Agora_memory::Alignment_t alignment, int init_zero)
{
    size_t size = dim * sizeof(T);
    *buffer = static_cast<T*>(Agora_memory::padded_aligned_alloc(alignment, size));
    if (init_zero) {
...
public:
    ...
    void alloc(size_t n_rows, size_t n_cols, size_t n_entries)
    {
        ...
        this->backing_buf_ = reinterpret_cast<T*>(Agora_memory::padded_aligned_alloc(Agora_memory::Alignment_t::k64Align, alloc_sz));
        std::memset(reinterpret_cast<uint8_t*>(this->backing_buf_), 0, alloc_sz);
    ...
    std::array<T*, COLS>& operator[](size_t row_idx) { return this->mat_[row_idx]; }
