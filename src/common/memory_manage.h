...
template <typename T> static void FreeBuffer1d(T** buffer)
{
    std::free(*buffer);
};
...
public:
    ...
    void Alloc(size_t n_rows, size_t n_cols, size_t n_entries)
    {
        ...
        this->backing_buf_
            = static_cast<T*>(Agora_memory::PaddedAlignedAlloc(
                Agora_memory::Alignment_t::kK64Align, alloc_sz));
        std::memset(
