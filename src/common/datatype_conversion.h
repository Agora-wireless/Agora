...
static inline void SimdConvertShortToFloat(
...
static inline void SimdConvertFloatToShort(const float* in_buf, short* out_buf,
    size_t n_elems, size_t cp_len, size_t scale_down_factor)
{
...
static inline void ConvertFloatTo12bitIq(
...
#ifdef __AVX512F__
static inline void simd_convert_16bit_iq_to_float(
    __m256i val, float* out_buf, __m512 magic, __m512i magic_i)
{
...
static inline void Convert12bitIqTo16bitIq(
    uint8_t* in_buf, uint16_t* out_buf, size_t n_elems)
{
    for (size_t i = 0; i < n_elems; i += 16) {
        ...
        __m256i temp_q = _mm256_setr_epi16(*(uint16_t*)(in_buf + 1),
            *(uint16_t*)(in_buf + 4), *(uint16_t*)(in_buf + 7),
            *(uint16_t*)(in_buf + 10), *(uint16_t*)(in_buf + 13),
            *(uint16_t*)(in_buf + 16), *(uint16_t*)(in_buf + 19),
            *(uint16_t*)(in_buf + 22), *(uint16_t*)(in_buf + 25),
            *(uint16_t*)(in_buf + 28), *(uint16_t*)(in_buf + 31),
            *(uint16_t*)(in_buf + 34), *(uint16_t*)(in_buf + 37),
            *(uint16_t*)(in_buf + 40), *(uint16_t*)(in_buf + 43),
            *(uint16_t*)(in_buf + 46));
...
static inline void SimdConvert12bitIqToFloat(const uint8_t* in_buf,
    float* out_buf, uint16_t* in_16bits_buf, size_t n_elems)
{
    ...
    for (size_t i = 0; i < n_elems / 3; i += 16) {
        ...
        __m256i output_1 = _mm256_permute2f128_si256(iq_0, iq_1, 0x31);
#ifdef __AVX512F__
        simd_convert_16bit_iq_to_float(
            output_0, out_buf + i * 2, magic, magic_i);
        simd_convert_16bit_iq_to_float(
            output_1, out_buf + i * 2 + 16, magic, magic_i);
#else
...
// in_buf and out_buf must be 64-byte aligned
// n_elems must be a multiple of 16
static inline void SimdConvertFloat16ToFloat32(
...
// in_buf and out_buf must be 64-byte aligned
// n_elems must be a multiple of 16
static inline void SimdConvertFloat32ToFloat16(
    float* out_buf, const float* in_buf, size_t n_elems)
{
#ifdef __AVX512F__
    for (size_t i = 0; i < n_elems; i += 16) {
