...
ProportionalFairness::ProportionalFairness( const size_t spatial_streams, 
            const size_t bss_num, const size_t ues_num , size_t ofdm_data_num_ ) : 
    selected_option_(0),
    ...
    channel_covariance_matrices_( bss_num_ , bss_num_, ues_num_, arma::fill::zeros ) {
    ...
    options_num_ = options_vector.size();
    
    schedule_buffer_.Calloc(options_num_, ues_num_ * ofdm_data_num_,
                            Agora_memory::Alignment_t::kAlign64);
    schedule_buffer_index_.Calloc(options_num_, spatial_streams_ * ofdm_data_num_,
                            Agora_memory::Alignment_t::kAlign64);

    //Schedule Buffer Process
    for( size_t gp = 0; gp < options_num_; gp++ ) {
        for (size_t sc = 0; sc < ofdm_data_num_; sc++) {
            std::vector<size_t> UEs_idx = options_vector[gp];
    ...
    if( kPrintSchedulingBuffers ) {
        for (size_t row = 0; row < options_num_; row++) {
...
void ProportionalFairness::Update( size_t frame_id, arma::cx_fmat csi, std::vector<float> snr_per_ue ) {
    std::vector<float> ues_capacity = UEsCapacity( csi , snr_per_ue );    
    UpdateScheduler( frame_id, ues_capacity );
}

size_t ProportionalFairness::UpdateScheduler( size_t frame_id, std::vector<float> ues_capacity ) {
    if( frame_id < kCalibFrames ) {
        selected_option_ = frame_id % options_num_;
        if( kPrintSchedulingOption ) {
            std::stringstream s_;
            s_ << "Scheduled PF Frame " << frame_id  << " Calibration " << frame_id << "/" << kCalibFrames << "\n";
        ...
        if( kPrintSchedulingOption ) {
            std::stringstream s_;
            s_ << "Scheduled PF Frame " << frame_id  << " Option " << selected_option_ << " = [ ";
...
std::vector<float> ProportionalFairness::UEsCapacity( arma::cx_fmat csi, std::vector<float> snr_per_ue ) {
    // Calculate the channel covariance matrices for each UE
    std::vector<size_t> selected_ues = options_vector[selected_option_];
    ...
    for (size_t ue = 0; ue < spatial_streams_; ue++) {   
        size_t ue_idx = selected_ues[ue];
        arma::cx_fmat capacity_matrix = arma::eye<arma::cx_fmat>(bss_num_, bss_num_) + snr_per_ue[ue_idx] * channel_covariance_matrices_.slice(ue_idx);
        capacities_per_ue_[ue_idx] = std::log2f( std::abs( arma::det( capacity_matrix ) ) );
    }
    return capacities_per_ue_;
}

void ProportionalFairness::Schedule( size_t frame , std::vector<float> ues_capacity ) {
    arma::vec pf_( options_num_, arma::fill::zeros );
    float max_pf = 0;
    for( size_t action = 0; action < options_num_; action++ ) {
        std::vector<size_t> selected_ues = options_vector[action];
        if( frame > 0 ) {
...
void ProportionalFairness::UpdatePF( size_t frame, std::vector<float> ues_capacity ) {
    for( size_t i = 0; i < options_vector[selected_option_].size(); i++) {
        size_t idx_ = options_vector[selected_option_][i];
        pf_ues_history[idx_] += ues_capacity[idx_];
    }
    for( size_t ue = 0; ue < ues_num_; ue++) {
        if(std::find(options_vector[selected_option_].begin(), options_vector[selected_option_].end(), ue) != options_vector[selected_option_].end()) {
            ues_flags_[ue] = true;
