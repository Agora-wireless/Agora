...
RB_Share::RB_Share(Config* const cfg)  // Initialization
  ...
  for (const auto& elem : read_data) {
    ...
    std::vector<int> temp(
        int_array,
        int_array + elem.len);  // Using the vector range constructor
  ...
  arma::vec cg_tti_rb_ue(dims_cg[0] * dims_cg[1] * dims_cg[2],
                         arma::fill::zeros);
...
void RB_Share::Update(size_t frame, const std::vector<arma::cx_fmat>& csi,
                      const std::vector<float>& snr_per_ue,
                      const std::vector<float>& last_throughput) {
  // csi shape: 1-D arma::cx_vec (Num_UE * Num_BS * Num_RBG);
  // total_slice_tp_: accumulated data rate of slices
  // avg_rb_ only for Multi-RB to compute parallel
  // total_rb_allocated_: count total # of allocated RBs over TTIs
  ...
  for (size_t i = 0; i < num_prbs_; i++) {
    for (size_t j = 0; j < cfg_->UeAntNum(); j++) {
      cg_rb_ue(i, j) = cg_tti_rb_ue_[est_frame * num_prbs_ * cfg_->UeAntNum() +
                                     i * cfg_->UeAntNum() + j];
    }
  }
  ...
  while (any_positive(delta_slice) && !RBG_remain.empty()) {
    ...
    for (int num = 0; num < cfg_->UeAntNum(); ++num) {
    ...
    if (!non_zero.empty()) {
      mean_delta = std::accumulate(non_zero.begin(), non_zero.end(), 0.0) /
                   non_zero.size();
    } else {
      // Calculate mean of delta_slice if non_zero is empty
      if (!delta_slice.empty()) {
        mean_delta =
            std::accumulate(delta_slice.begin(), delta_slice.end(), 0.0) /
            delta_slice.size();
      }
    }
    // Define Parallel
    if (new_rb_para) {
      if (frame > 1) {
        float max_non_zero =
            *std::max_element(non_zero.begin(), non_zero.end());
        parallel =
            static_cast<int>((max_non_zero * non_zero.size() / avg_rb_)) + 1;
    ...
    std::partial_sum(Num_UE_ps.begin(), Num_UE_ps.end(),
                     prefix_sum.begin() + 1);
    ...
    arma::mat cg_large(RBG_remain.size(), large_ue_list.size(),
                       arma::fill::zeros);
    ...
    if (parallel == 1) {
      ...
      if (est_frame == 0 && sel_rbg == 0) {
        ...
        sum_lower = arma::sum(
            group_len_.subvec(0, est_frame * num_prbs_ + sel_rbg - 1));
      }

      int sum_upper =
          arma::sum(group_len_.subvec(0, est_frame * num_prbs_ + sel_rbg));

      std::vector<std::vector<int>> group_target(
          group_vector_.begin() + sum_lower, group_vector_.begin() + sum_upper);

      // std::vector<std::vector<int>> group_target = group_vector_;

      auto [sel_UE_list, cap_per_ue] =
          alloc_rb(group_target, cg_ue_vec, H_t, large_ue_list, small_ue_list,
                   ue_index, cfg_->SpatialStreamsNum());
      ...
      avg_rb_ =
          std::accumulate(cap_per_ue.begin(), cap_per_ue.end(), 0.0) / parallel;
      ...
      for (size_t count = 0; count < parallel; count++) {
        ...
        std::vector<std::vector<int>> group_target(
            group_vector_.begin() + sum_lower,
            group_vector_.begin() + sum_upper);

        // std::vector<std::vector<int>> group_target = group_vector_;

        // std::cout << "Before alloc_rb Good:" << count << std::endl;
        auto [sel_UE_list, cap_per_ue] =
            alloc_rb(group_target, cg_ue_vec, H_t, large_ue, small_ue, ue_index,
        ...
        RBG_remain.erase(RBG_remain.begin() + rbg_index);
        total_rate +=
            std::accumulate(cap_per_ue.begin(), cap_per_ue.end(), 0.0);
  ...
  // ************************* For Multi-RB *************************************
  std::vector<float> slice_tp(Num_slice);

  for (size_t i = 0; i < Num_slice; i++) {
    slice_tp[i] = est_slice_tp_[i] - total_slice_tp_[i];
  }
  float tp_tti = std::accumulate(slice_tp.begin(), slice_tp.end(), 0.0);
  float current_alloc_rb = num_prbs_ - RBG_remain.size();
  avg_rb_ =
      tp_tti / current_alloc_rb;  // Update avg_rb_ to compute parallel next TTI
  // ************************* For Multi-RB *************************************
...
std::pair<std::vector<int>, arma::fvec> RB_Share::alloc_rb(
    ...
    for (auto it = group_list.begin(); it != group_list.end(); ++it) {
      ...
      if (std::find(group.begin(), group.end(), ue_index) != group.end()) {
        if (group.size() <= len_remain) {
          for (int ele : group) {
            if (std::find(sel_UE_list.begin(), sel_UE_list.end(), ele) ==
                sel_UE_list.end()) {
              auto idx =
                  std::find(large_ue_list.begin(), large_ue_list.end(), ele);
              auto idx_1 =
                  std::find(small_ue_list.begin(), small_ue_list.end(), ele);
      ...
      if (!large_ue_list.empty()) {
        ...
        for (int index : large_ue_list) {
          if (cg_ue_vec(index) > max_value) {
            ...
            max_index =
                index;  // Store the actual index within the original vector
          }
...
bool RB_Share::IsUeScheduled(size_t frame_id, size_t prb_id, size_t ue_id) {
  size_t gp = frame_id % num_groups_;
  return (schedule_buffer_[gp][ue_id + cfg_->UeAntNum() * prb_id] != 0);
}

arma::uvec RB_Share::ScheduledUeMap(size_t frame_id, size_t prb_id) {
  size_t gp = frame_id % num_groups_;
...
arma::uvec RB_Share::ScheduledUeList(size_t frame_id, size_t prb_id) {
  size_t gp = frame_id % num_groups_;
...
size_t RB_Share::GetGroup(size_t frame_id) { return frame_id % num_groups_; }
