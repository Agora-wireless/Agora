...
RadioTXRX::RadioTXRX(Config* cfg, int n_threads, int in_core_id)
  ...
  if (!kUseArgos && !kUseUHD) {
    socket_.resize(config_->num_radios());
    servaddr_.resize(config_->num_radios());
  } else {
...
bool RadioTXRX::startTXRX(Table<char>& in_buffer, Table<int>& in_buffer_status,
  ...
  for (int i = 0; i < thread_num_; i++) {
    ...
    context->id = i;
    // start socket thread
    if ((kUseArgos == true) && (config_->hw_framer() == true)) {
      if (pthread_create(
              &txrx_thread, NULL,
...
int RadioTXRX::dequeue_send(int tid) {
  ...
  if (event.event_type == EventType::kPacketTX) {
    for (size_t symbol_id = 0; symbol_id < c->frame().NumULSyms();
         symbol_id++) {
      size_t offset =
          (c->GetTotalDataSymbolIdxUl(frame_id, symbol_id) * c->ue_ant_num()) +
          ant_id;
    ...
    rt_assert(
        message_queue_->enqueue(
            *rx_ptoks_[tid], Event_data(EventType::kPacketTX, event.tags[0])),
...
void* RadioTXRX::loop_tx_rx(int tid) {
  ...
  size_t rx_offset = 0;
  int radio_lo = tid * config_->num_radios() / thread_num_;
  int radio_hi = (tid + 1) * config_->num_radios() / thread_num_;
...
int RadioTXRX::dequeue_send_argos(int tid, long long time0) {
  ...
  int num_samps = c->samps_per_symbol();
  ...
  int flags_tx_pilot = (kUseUHD && c->num_channels() == 2) ? 1 : 2;
  ...
  size_t ant_id = ue_id * c->num_channels();
  long long txTime(0);
  int r;
  ...
  if (!c->hw_framer()) {
    ...
    txTime = time0 + tx_frame_id * frm_num_samps + pilot_symbol_id * num_samps -
             c->cl_tx_advance();
    r = radio->radioTx(ue_id, pilot_buff0.data(), num_samps, flags_tx_pilot,
                       txTime);
    if (r < num_samps)
  ...
  for (size_t symbol_id = 0; symbol_id < c->frame().NumULSyms(); symbol_id++) {
    ...
    void* txbuf[2];
    for (size_t ch = 0; ch < c->num_channels(); ++ch) {
      struct Packet* pkt =
...
struct Packet* RadioTXRX::recv_enqueue_argos(int tid, size_t radio_id,
  ...
  long long rxTime(0);
  struct Packet* pkt[c->num_channels()];
  void* samp[c->num_channels()];
  for (size_t ch = 0; ch < c->num_channels(); ++ch) {
    pkt[ch] =
  ...
  size_t ant_id = radio_id * c->num_channels();
  for (size_t ch = 0; ch < c->num_channels(); ++ch) {
    ...
    Event_data rx_message(EventType::kPacketRX,
                          rx_tag_t(tid, rx_offset + ch)._tag);
...
void* RadioTXRX::loop_tx_rx_argos(int tid) {
  pin_to_core_with_offset(ThreadType::kWorkerTXRX, core_id_, tid);
  auto& c = config_;
  size_t num_radios = c->num_radios();
  ...
  while (c->running() == true) {
    ...
    // transmit data
    if (-1 != dequeue_send_argos(tid, 0)) continue;

    struct Packet* pkt =
        recv_enqueue_argos(tid, radio_id, frame_id, symbol_id, rx_offset);
    if (pkt == NULL) continue;

    rx_offset += c->num_channels();
...
void* RadioTXRX::loop_tx_rx_argos_sync(int tid) {
  ...
  std::vector<std::complex<int16_t>> zeros0(c->samps_per_symbol(), 0);
  std::vector<std::complex<int16_t>> zeros1(c->samps_per_symbol(), 0);
  ...
  while ((c->running() == true) && sync_index < 0) {
    ...
    for (size_t find_beacon_retry = 0;
      ...
      r = radio->radioRx(radio_id, frm_rx_buff.data(), frm_num_samps, rxTime);
    ...
    rx_offset = sync_index - c->beacon_len() - c->ofdm_tx_zero_prefix();
  }
  ...
  while (c->running() == true) {
    ...
    for (size_t symbol_id = 1; symbol_id < c->frame().NumTotalSyms();
      ...
      if (!config_->IsPilot(frame_id, symbol_id) &&
        ...
        struct Packet* pkt =
            recv_enqueue_argos(tid, radio_id, frame_id, symbol_id, cursor);
        if (pkt == NULL) break;

        cursor += c->num_channels();
