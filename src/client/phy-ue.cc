...
PhyUe::PhyUe(Config* config) {
  ...
  for (size_t i = 0; i < config_->ue_ant_num_; i++) {
    for (size_t j = config->ofdm_tx_zero_prefix_;
         j < config_->samps_per_symbol_ - config->ofdm_tx_zero_postfix_; j++) {
  ...
  encode_queue_ =
      moodycamel::ConcurrentQueue<EventData>(kFrameWnd * config_->ue_num_ * 36);
  modul_queue_ =
      moodycamel::ConcurrentQueue<EventData>(kFrameWnd * config_->ue_num_ * 36);
  ifft_queue_ =
      moodycamel::ConcurrentQueue<EventData>(kFrameWnd * config_->ue_num_ * 36);
  ...
  for (size_t i = 0; i < kFrameWnd; i++) {
    ...
    std::memset(fft_checker_[i], 0, sizeof(size_t) * (config_->ue_ant_num_));
  }
  ...
  if (dl_data_symbol_perframe_ > 0) {
    for (size_t i = 0; i < kFrameWnd; i++) {
      ...
      std::memset(demul_checker_[i], 0,
                  sizeof(size_t) * (config_->ue_ant_num_));
    }
  }
        ...
        case EventType::kPacketRX: {
          ...
          if ((symbol_id == 0)  // Beacon in Sim mode!
            ...
            EventData do_tx_pilot_task(
                ...
                gen_tag_t::FrmSymUe(frame_id, config_->pilot_symbols_[0][ue_id],
                                    ue_id)
                    .tag_);
  ...
  if (kPrintDownlinkPilotStats) {
    if (config_->IsPilot(frame_id, symbol_id)) {
      ...
      if (frame_id == kRecordFrameIndex) {
        ...
        fwrite(pkt->data_, 2 * sizeof(int16_t), config_->samps_per_symbol_, f);
        fclose(f);
      }
  ...
  if (dl_symbol_id < dl_pilot_symbol_perframe_) {
    ...
    for (size_t j = 0; j < config_->ofdm_data_num_; j++) {
      if (j % config_->ofdm_pilot_spacing_ != 0) {
        ...
        complex_float tx =
            config_->dl_iq_f_[dl_symbol_id][ant_id * config_->ofdm_ca_num_ +
    ...
    if (kPrintEqualizedSymbols) {
      ...
      arma::cx_fvec equal_vec(equ_buffer_ptr, config_->ofdm_data_num_, false);
      Utils::PrintVec(equal_vec, std::string("equ") +
                                     std::to_string(total_dl_symbol_id) +
                                     std::string("_") + std::to_string(ant_id));
    }
    evm =
        std::sqrt(evm) / (config_->ofdm_data_num_ - config_->GetOfdmPilotNum());
    if (kPrintPhyStats) {
      std::stringstream ss;
...
void PhyUe::DoDecode(int tid, size_t tag) {
  ...
  size_t total_dl_symbol_id = frame_slot * dl_data_symbol_perframe_ +
                              dl_symbol_id - dl_pilot_symbol_perframe_;
  size_t symbol_ant_offset = total_dl_symbol_id * config_->ue_ant_num_ + ant_id;
...
void PhyUe::DoEncode(int tid, size_t tag) {
  ...
  int8_t* encoded_buffer_temp =
      static_cast<int8_t*>(Agora_memory::PaddedAlignedAlloc(
          ...
          LdpcEncodingEncodedBufSize(cfg->ldpc_config_.bg_,
                                     cfg->ldpc_config_.zc_)));
  int8_t* parity_buffer = static_cast<int8_t*>(Agora_memory::PaddedAlignedAlloc(
      Agora_memory::Alignment_t::kK64Align,
      LdpcEncodingParityBufSize(cfg->ldpc_config_.bg_, cfg->ldpc_config_.zc_)));
    ...
    for (size_t cb_id = 0; cb_id < config_->ldpc_config_.nblocks_in_symbol_;
      ...
      if (kEnableMac) {
        ...
        size_t cb_offset =
            ...
            bytes_per_block;
        input_ptr =
            &cfg->ul_bits_[ul_symbol_id + config_->ul_pilot_syms_][cb_offset];
      }

      LdpcEncodeHelper(ldpc_config.bg_, ldpc_config.zc_, ldpc_config.n_rows_,
                       encoded_buffer_temp, parity_buffer, input_ptr);

      int cb_coded_bytes = ldpc_config.cb_codew_len_ / cfg->mod_order_bits_;
      int output_offset =
          total_ul_symbol_id * config_->ofdm_data_num_ + cb_coded_bytes * cb_id;

      AdaptBitsForMod(reinterpret_cast<uint8_t*>(encoded_buffer_temp),
                      &ul_syms_buffer_[ue_id][output_offset],
                      encoded_bytes_per_block, cfg->mod_order_bits_);
    }
  }
  ...
  for (size_t ch = 0; ch < config_->n_channels_; ch++) {
    ...
    for (size_t ul_symbol_id = 0; ul_symbol_id < ul_data_symbol_perframe_;
      ...
      complex_float* modul_buf =
          &modul_buffer_[total_ul_symbol_id][ant_id * config_->ofdm_data_num_];
      int8_t* ul_bits =
...
void PhyUe::DoIfft(int tid, size_t tag) {
  ...
  for (size_t ch = 0; ch < config_->n_channels_; ch++) {
    ...
    for (size_t ul_symbol_id = 0; ul_symbol_id < ul_symbol_perframe_;
      ...
      size_t buff_offset = total_ul_symbol_id * config_->ue_ant_num_ + ant_id;
      ...
      if (ul_symbol_id < config_->ul_pilot_syms_) {
        ...
        size_t total_ul_data_symbol_id = frame_slot * ul_data_symbol_perframe_ +
                                         ul_symbol_id - config_->ul_pilot_syms_;
...
void PhyUe::InitializeUplinkBuffers() {
  ...
  ul_bits_buffer_.Malloc(config_->ue_ant_num_, ul_bits_buffer_size_,
                         Agora_memory::Alignment_t::kK64Align);
  ul_bits_buffer_status_.Calloc(config_->ue_ant_num_, kFrameWnd,
                                Agora_memory::Alignment_t::kK64Align);
...
void PhyUe::InitializeDownlinkBuffers() {
  ...
  if (dl_data_symbol_perframe_ > 0) {
    ...
    dl_demod_buffer_.Calloc(buffer_size, config_->ofdm_data_num_ * kMaxModType,
                            Agora_memory::Alignment_t::kK64Align);
    ...
    decoded_bits_count_.Calloc(config_->ue_ant_num_, task_buffer_symbol_num_dl,
                               Agora_memory::Alignment_t::kK64Align);
    ...
    decoded_blocks_count_.Calloc(config_->ue_ant_num_,
                                 ...
                                 Agora_memory::Alignment_t::kK64Align);
    block_error_count_.Calloc(config_->ue_ant_num_, task_buffer_symbol_num_dl,
                              Agora_memory::Alignment_t::kK64Align);
    ...
    std::memset(decoded_symbol_count_, 0,
                sizeof(size_t) * config_->ue_ant_num_);
    std::memset(symbol_error_count_, 0, sizeof(size_t) * config_->ue_ant_num_);
  }
}
