...
ClientRadioConfig::ClientRadioConfig(Config* cfg) : _cfg(cfg) {
...
void ClientRadioConfig::initClientRadio(ClientRadioConfigContext* in_context) {
  ...
  if (!kUseUHD) clStn[i]->writeSetting("RESET_DATA_LOGIC", "");
  ...
  for (auto ch : channels) {
    clStn[i]->setBandwidth(SOAPY_SDR_RX, ch, cfg->bw_filter());
    clStn[i]->setBandwidth(SOAPY_SDR_TX, ch, cfg->bw_filter());
  ...
  for (auto ch : channels) {
    ...
    if (!kUseUHD) clStn[i]->setDCOffsetMode(SOAPY_SDR_RX, ch, true);
  }
...
bool ClientRadioConfig::radioStart() {
  ...
  for (size_t r = 0; r < _radioNum; r++) {
    ...
    for (size_t s = 0; s < _cfg->frame().frame_identifier().length(); s++) {
      ...
      if (c == 'P' and ((_cfg->num_channels() == 1 and
                         _cfg->frame().GetPilotSymbol(r) != s) or
                        (_cfg->num_channels() == 2 and
                         (_cfg->frame().GetPilotSymbol(2 * r) != s and
                          _cfg->frame().GetPilotSymbol(r * 2 + 1) !=
  ...
  for (size_t i = 0; i < this->_radioNum; i++) {
    if (_cfg->hw_framer()) {
      ...
      int max_frame_ =
          (int)(2.0 /
                ((_cfg->samps_per_symbol() * _cfg->frame().NumTotalSyms()) /
                 _cfg->rate()));
      conf["max_frame"] = max_frame_;
      conf["dual_pilot"] = (_cfg->num_channels() == 2);
      ...
      clStn[i]->writeSetting("CORR_START",
                             (_cfg->channel() == "B") ? "B" : "A");
    } else {
      if (!kUseUHD) {
        ...
        clStn[i]->activateStream(this->rxStreams[i], SOAPY_SDR_HAS_TIME, 1e9,
                                 0);
        clStn[i]->activateStream(this->txStreams[i], SOAPY_SDR_HAS_TIME, 1e9,
                                 0);
      }
    }
  }
...
int ClientRadioConfig::radioTx(size_t r /*radio id*/, void** buffs,
  ...
  if (kDebugRadioTX) {
    ...
    long timeoutUs(0);
    int statusFlag = 0;
    int s = clStn[r]->readStreamStatus(this->txStreams[r], chanMask, statusFlag,
                                       frameTime, timeoutUs);
...
int ClientRadioConfig::radioRx(size_t r /*radio id*/, void** buffs,
  ...
  if (r < this->_radioNum) {
    int ret(0);
    if (_cfg->hw_framer()) {
      ret = clStn[r]->readStream(this->rxStreams[r], buffs, num_samps, flags,
                                 frameTime, 1000000);
    } else {
...
void ClientRadioConfig::drain_buffers() {
  ...
  for (size_t i = 0; i < _cfg->num_radios(); i++) {
    ClientRadioConfig::drain_rx_buffer(clStn[i], rxStreams[i], dummybuffs,
...
void ClientRadioConfig::drain_rx_buffer(SoapySDR::Device* dev,
  ...
  long timeoutUs(0);
  while (r != -1) {
    r = dev->readStream(istream, buffs.data(), symSamp, flags, frameTime,
                        timeoutUs);
    i++;
  }
...
void ClientRadioConfig::readSensors() {
  for (size_t i = 0; i < this->_radioNum; i++) {
    ...
    std::cout << "TX0 TEMP  : " << clStn[i]->readSensor(SOAPY_SDR_TX, 0, "TEMP")
              << std::endl;
    std::cout << "TX1 TEMP  : " << clStn[i]->readSensor(SOAPY_SDR_TX, 1, "TEMP")
              << std::endl;
    std::cout << "RX0 TEMP  : " << clStn[i]->readSensor(SOAPY_SDR_RX, 0, "TEMP")
              << std::endl;
    std::cout << "RX1 TEMP  : " << clStn[i]->readSensor(SOAPY_SDR_RX, 1, "TEMP")
              << std::endl;
    std::cout << std::endl;
  }
}
