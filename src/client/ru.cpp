...
std::vector<pthread_t> RU::startProc(Table<char>& in_buffer,
...
void RU::sendThread(int tid)
{
    ...
    while (config_->running) {
        ...
        // for (int p_id = 0; p_id < cfg->pilotSymsPerFrame; p_id++)
        {
            ...
            if (sendto(tx_socket_[tid], (char*)obj_ptr->pilot_buffer_,
                    packet_length, 0, (struct sockaddr*)&obj_ptr->cliaddr_[tid],
                    sizeof(obj_ptr->cliaddr_[tid]))
                < 0) {
...
#if DEBUG_SEND
            printf("RU: transmit tx_frame_id %d, tx_symbol_id %d, cell_id %d, "
                   "ant_id %d\n",
...
void RU::taskThread(int tid)
{
    ...
    while (config_->running) {
        ...
        for (int it = radio_start; it < radio_end;
             it++) // FIXME: this must be threaded
        {
            ...
            if (txSymbols.size() > 0
                ...
                //                do_tx_task.more_data = frame_id +
                //                TX_FRAME_DELTA; if (
                //                !task_queue_.enqueue(*task_ptok[tid],
                //                do_tx_task)) {
                //                    printf("task enqueue failed\n");
                //                    exit(0);
                //                }
                ...
                for (size_t tx_symbol_id = 0; tx_symbol_id < txSymbols.size();
                    ...
                    // int tx_offset = generateOffset3d(TASK_BUFFER_FRAME_NUM,
                    // txSymbols.size(), config_->getNumAntennas(), tx_frame_id,
                    // tx_symbol_id, ant_id);
                    int tx_packet_length
                        = config_->packet_length - offsetof(Packet, data);
                    int frame_samp_size
                        = tx_packet_length * n_ant * txSymbols.size();
                    int tx_offset = tx_frame_offset * frame_samp_size
                        + tx_packet_length * (n_ant * tx_symbol_id + ant_id);
                    void* txbuf[2];
                    long long frameTime
