...
void ClientRadioConfig::InitClientRadio(ClientRadioConfigContext* in_context)
{
    ...
    rx_streams_[i] = cl_stn_[i]->setupStream(
        SOAPY_SDR_RX, SOAPY_SDR_CS16, channels, sargs);
    tx_streams_[i] = cl_stn_[i]->setupStream(
    ...
    for (auto ch : channels) {
        ...
        cl_stn_[i]->setFrequency(
            SOAPY_SDR_RX, ch, "BB", kUseUHD ? 0 : cfg->nco_);
        cl_stn_[i]->setFrequency(SOAPY_SDR_TX, ch, "RF", cfg->radio_rf_freq_);
        cl_stn_[i]->setFrequency(
            SOAPY_SDR_TX, ch, "BB", kUseUHD ? 0 : cfg->nco_);
        ...
        if (!kUseUHD) {
            ...
            if (cfg_->SingleGain()) {
                ...
                cl_stn_[i]->setGain(SOAPY_SDR_TX, ch, "PAD",
                    ch != 0u ? cfg->tx_gain_b_ + cfg->client_gain_adj_b_[i]
                             : cfg->tx_gain_a_
                            + cfg->client_gain_adj_a_[i]); //[0,30]
            }
        } else {
...
bool ClientRadioConfig::RadioStart()
{
    ...
    for (size_t r = 0; r < radio_num_; r++) {
        ...
        for (size_t s = 0; s < cfg_->frames_[0].size(); s++) {
            ...
            if (c == 'P'
                and ((cfg_->n_channels_ == 1
                         and cfg_->pilot_symbols_[0][r] != s)
                    or (cfg_->n_channels_ == 2
                        and (cfg_->pilot_symbols_[0][2 * r] != s
                            and cfg_->pilot_symbols_[0][r * 2 + 1]
    ...
    for (size_t i = 0; i < this->radio_num_; i++) {
        if (cfg_->hw_framer_) {
            ...
            int max_frame = (int)(2.0
                / ((cfg_->samps_per_symbol_ * cfg_->symbol_num_perframe_)
                    / cfg_->rate_));
            ...
            cl_stn_[i]->setHardwareTime(
                SoapySDR::ticksToTimeNs(
                    (sf_start << 16) | sp_start, cfg_->rate_),
                "TRIGGER");
            cl_stn_[i]->writeSetting("TX_SW_DELAY",
            ...
            cl_stn_[i]->writeSetting(
                "CORR_START", (cfg_->channel_ == "B") ? "B" : "A");
        } else {
            if (!kUseUHD) {
...
void ClientRadioConfig::DrainBuffers()
{
    ...
    for (size_t i = 0; i < cfg_->n_radios_; i++) {
        ClientRadioConfig::DrainRxBuffer(
            cl_stn_[i], rx_streams_[i], dummybuffs, cfg_->samps_per_symbol_);
    }
}
...
void ClientRadioConfig::ReadSensors()
{
    for (size_t i = 0; i < this->radio_num_; i++) {
        ...
        std::cout << "FE_TEMP  : " << cl_stn_[i]->readSensor("FE_TEMP")
                  << std::endl;
        std::cout << "TX0 TEMP  : "
                  << cl_stn_[i]->readSensor(SOAPY_SDR_TX, 0, "TEMP")
                  << std::endl;
        std::cout << "TX1 TEMP  : "
                  << cl_stn_[i]->readSensor(SOAPY_SDR_TX, 1, "TEMP")
                  << std::endl;
        std::cout << "RX0 TEMP  : "
                  << cl_stn_[i]->readSensor(SOAPY_SDR_RX, 0, "TEMP")
                  << std::endl;
        std::cout << "RX1 TEMP  : "
                  << cl_stn_[i]->readSensor(SOAPY_SDR_RX, 1, "TEMP")
                  << std::endl;
        std::cout << std::endl;
    }
}
