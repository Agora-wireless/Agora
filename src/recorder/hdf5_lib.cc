...
int Hdf5Lib::createDataset(H5std_string dataset_name,
  ...
  try {
    ...
    file_->createDataSet(ds_name, H5::PredType::STD_I16BE, ds_dataspace,
                         ds_prop);
    ds_prop.close();
  }
  ...
  for (size_t i = 0; i < dataset_str_.size(); i++) {
    ...
    try {
      ...
      if (kPrintDataSetInfo == true) {
        ...
        using std::cout;
        cout << "dim " + dataset_str_.at(i) + " chunk = " << cndims
             << std::endl;
        cout << "New " + dataset_str_.at(i) + " Dataset Dimension: [";
        for (size_t j = 0; j < kDsDimsNum - 1; ++j)
          cout << dims_.at(i).at(j) << ",";
        cout << dims_.at(i).at(kDsDimsNum - 1) << "]" << std::endl;
      }
      filespace.close();
    }
    // catch failure caused by the H5File operations
