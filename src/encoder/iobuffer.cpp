...
void Adapter64to256(int8_t* ptr_buff_0, int8_t* ptr_buff_1, uint16_t zc_size,
  ...
  // the __m256i data type can hold thirty-two 8-bit, sixteen 16-bit, eight
  // 32-bit, or four 64-bit integer values.  The compiler aligns the __m256,
  ...
  if (kPrintAdapterDebug) {
    std::printf(
        ...
        "iterations %d\n",
        direct == 1, cb_len_bits, zc_size, (cb_len_bits / zc_size));
  }
