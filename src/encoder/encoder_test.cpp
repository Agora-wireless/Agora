...
int main()
{
    // __declspec (align(PROC_BYTES)) int8_t internalBuffer0[BG1_ROW_TOTAL *
    // PROC_BYTES] = {0};
    // __declspec (align(PROC_BYTES)) int8_t internalBuffer1[BG1_ROW_TOTAL *
    // PROC_BYTES] = {0};
    // __declspec (align(PROC_BYTES)) int8_t internalBuffer2[BG1_COL_TOTAL *
    // PROC_BYTES] = {0};
    ALIGNED_(PROC_BYTES)
    ...
    for (int i = 0; i < numberCodeblocks; i++) {
        ...
        parity[i]
            = (int8_t*)malloc(BG1_ROW_TOTAL * PROC_BYTES * sizeof(int8_t));
        output[i]
            = (int8_t*)malloc(BG1_COL_TOTAL * PROC_BYTES * sizeof(int8_t));
        reference[i]
            = (int8_t*)read_binfile(reference_filename, (cbEncLen + 7) >> 3);
    }
    ...
    for (int n = 0; n < numberCodeblocks; n++) {
        ...
        // this is for testing only because intel's reference files only provide
        // the parity bits
    ...
    for (int n = 0; n < numberCodeblocks; n++) {
        ...
        for (int i = 0; i < (cbEncLen >> 3); i++) {
            ...
            // printf("%d: (%i %i), error: %i\n", i, reference_buffer[i],
            // parity_buffer[i], error);
            for (int j = 0; j < 8; j++) {
                err_cnt += error & 1;
                error >>= 1;
            }
