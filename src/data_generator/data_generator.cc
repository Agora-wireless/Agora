...
void DataGenerator::DoDataGeneration(const std::string& directory) {
  ...
  for (size_t frame_id = 0; frame_id < kNumGeneratedFrames; frame_id++) {
    ...
    for (size_t i = 0; i < this->cfg_->Frame().NumTotalSyms(); i++) {
      ...
      const auto write_status = std::fwrite(ptr, this->cfg_->OfdmCaNum() * this->cfg_->BsAntNum() * 2,
                  sizeof(float), fp_rx);
      if (write_status != this->cfg_->OfdmCaNum() * this->cfg_->BsAntNum() * 2) {
        AGORA_LOG_ERROR("Wrote %zu out of %zu to file %s\n", write_status,
                        this->cfg_->OfdmCaNum() * this->cfg_->BsAntNum() * 2, filename_rx.c_str().c_str());
        throw std::runtime_error("Failed to write to file" +
                                 filename_rx);
      }
    }
